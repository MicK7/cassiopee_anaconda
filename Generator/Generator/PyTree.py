...
def adaptOctree(a, indicator='indicator', balancing=1, ratio=2):
    ...
    o = Internal.getZones(a)
    if len(o) != 1: print 'Warning: adaptOctree: more than 1 zone in argument. Only the first zone is kept.'
    o = o[0]; name = o[0]
    indicator = indicator.split(':')
    if len(indicator) == 2: indicator = indicator[1]
    else: indicator = indicator[0]
    hexa = C.getFields(Internal.__GridCoordinates__, o)[0]
    indic = C.getFields(Internal.__FlowSolutionCenters__, o)[0]
    indic = Converter.extractVars(indic, [indicator])
    res = Generator.adaptOctree(hexa, indic, balancing, ratio)
    return C.convertArrays2ZoneNode(name, [res])

def expandLayer(o, level=0, corners=0, balancing=0):
    name = o[0]
    hexa = C.getFields(Internal.__GridCoordinates__, o)[0]
    res = Generator.expandLayer(hexa, level, corners, balancing)
    return C.convertArrays2ZoneNode(name, [res])
...
def bboxIntersection(z1, z2, tol=1.e-6, isBB=False, method='AABB'):
    ...
    if method == 'AABB':  # Computes the intersection between 2 AABB
        ...
        return generator._obboxIntersection(z1, z2)          
