...
#pragma omp parallel for default(shared)
  ...
  E_Float etax2[nic];  E_Float etay2[nic];  E_Float etaz2[nic];  
  while (iter < niter)
  {
    iter++;
    for (E_Int i = 0; i < nic; i++)
                                   ...
                                   FldArrayF& vect, E_Float toldist)
{
  ...
  E_Float vxt[ni];  E_Float vyt[ni]; E_Float vzt[ni];
  ...
  while (iter < niter)
  {
    ...
    vx = vxt; vy = vyt; vz = vzt;
    // normalize
    normalize(ni, vx, vy, vz);
    applyConstraintsOnVector(ni, xt, yt, zt, loop, constrainedPts, constraints, vx, vy, vz, toldist);
    relaxNearConstraints(ni, xt, yt, zt, loop, constrainedPts, alpt, vx, vy, vz, dalphamax);
  }//fin boucle while
