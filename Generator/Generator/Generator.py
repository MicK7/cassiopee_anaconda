...
def bbox(arrays):
    ...
    for a in ars:
        ...
        xmin0 = C.getArgMin(a, varx)[0] 
        ymin0 = C.getArgMin(a, vary)[1]
        zmin0 = C.getArgMin(a, varz)[2]
        xmax0 = C.getArgMax(a, varx)[0]
        ymax0 = C.getArgMax(a, vary)[1]
        zmax0 = C.getArgMax(a, varz)[2]
...
def snapSharpEdges(meshes, surfaces, step=None, angle=30.):
    ...
    import Converter as C
    if len(out) != 1: contours = out[1]
    else: contours = None
    if len(out) == 3: corners = out[2]
    else: corners = None
...
def refinedSharpEdges__(surfaces, step, angle):
    ...
    if step is not None:
        for c in contours:
            ...
            l = D.getLength(c)
            N = int(l/step)+1
            if N == 1: N = 2 # pas de division par zero
    ...
    outList = [b]
    if ncontours != []: outList.append(contours)
    if corners != []: outList.append(corners) 
    return outList
