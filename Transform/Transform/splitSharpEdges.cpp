...
{ 
  ...
  char* isVisited = (char*)calloc(ne, sizeof(char)); // elt deja visite?
  E_Int* mustBeVisited = (E_Int*)malloc(ne * sizeof(E_Int));
  E_Int mbv, p, i, ie, elt, curr, necurr, lt;
  unsigned int iv;
  ...
  while (nev < ne)
  {
    ...
    while (mbv > 0)
    {
      ...
      pc += lt+1;
      curr += lt+1; necurr++;
      // recupere les n vertex de l'element
      K_CONNECT::getVertexIndices(cn->begin(), posFaces.begin(), pos[elt], indices);
      nvert = indices.size();
      pts1.reserve(nvert);
      ...
      for (iv = 0; iv < cEEN[elt].size(); iv++)
      {
        ...
        if (isVisited[ie] == 0)
        {
          // recupere n vertex de l'element
          K_CONNECT::getVertexIndices(cn->begin(), posFaces.begin(), pos[ie], indices);
          nvert = indices.size();
          pts2.reserve(nvert);
          for (E_Int k = 0; k < nvert; k++)
          {
