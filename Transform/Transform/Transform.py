...
def collapse(array):
    ...
    if isinstance(array[0], list): 
        ...
        return transform.collapse(array)
...
def rotate(array, center, arg1, arg2=None, vectors=[]):
    if arg2 is None: # kind of euler angles
        return rotate3__(array, center, arg1, vectors)
    elif isinstance(arg2, float) or isinstance(arg2, int):
        return rotate1__(array, center, arg1, arg2, vectors)
    else: return rotate2__(array, center, arg1, arg2, vectors)
...
def homothety(array, center, alpha):
    """Make for a mesh defined by an array an homothety of center Xc and
    of factor alpha.
    Usage: homothety(array, (xc,yc,zc), alpha)"""
    if isinstance(array[0], list): 
        b = []
        for i in array:
        ...
        return transform.homothety(array, center, alpha)
    
def contract(array, center, dir1, dir2, alpha):
    ...
    if isinstance(array[0], list): 
        ...
        return transform.contract(array, center, dir1, dir2, alpha)
...
def symetrize(array, point, vector1, vector2):
    """Make a symetry of mesh from plane passing by point and of director vector: vector1 and vector2.
    Usage: symetrize(array, (xc,yc,zc), (v1x,v1y,v1z), (v2x,v2y,v2z))"""
    if isinstance(array[0], list):
        b = []
        for i in array:
        ...
        return transform.symetrize(array, point, vector1, vector2)

def perturbate(array, radius, dim=3):
    ...
    if isinstance(array[0], list): 
        ...
        return transform.perturbate(array, radius, dim)
...
def smooth(a, eps=0.5, niter=4, type=0, fixedConstraints=[], 
    ...
    if isinstance(a[0], list):
        ...
        if listeType == 1: # all struct    
            ...
            for noz in xrange(len(a)):
                ...
                for ind in xrange(ninjnk):
                    indu = indicesU[ind]
                    coords[1][0,ind] = c[1][0,indu]
                    coords[1][1,ind] = c[1][1,indu]
                    coords[1][2,ind] = c[1][2,indu]
    ...
    elif len(a) == 5: # 1 array structure    
...
def deform(array, vector):
    """Deform surface by moving surface of the vector dx, dy, dz. 
    Usage: deform(array, vector)"""
    if isinstance(array[0], list):
        raise TypeError("deform: not for a list of arrays.")
    return transform.deform(array, vector)
...
def deformPoint(array, xyz, dxdydz, depth, width):
    ...
    Usage: deform(array, (x,y,z), (dx,dy,dz), depth, width)"""
    if isinstance(array[0], list):
        b = []
        for i in array:
        ...
        return transform.deformPoint(array, xyz, dxdydz, depth, width)

def deformMesh(array, surfDelta, beta=4.):
    """Deform a mesh wrt surfDelta defining surface grids and deformation vector on it.
    Usage: deformMesh(array, surfDelta, beta)"""
    if not isinstance(array[0], list):
        if len(array) == 5: return deformMeshStruct__(array, surfDelta, beta)
        else: raise TypeError("deformMesh: not valid for unstructured arrays.")
    else:
        out = []
        for i in array:
            if len(i) == 5: out.append(deformMeshStruct__(i, surfDelta, beta))
            else: raise TypeError("deformMesh: not valid for unstructured arrays.")
        return out

def deformMeshStruct__(arrayi, surfDelta, beta):
    try: import Generator as G
    ...
    #Converter.convertArrays2File(borders,'borders.plt')
    #import sys; sys.exit()
    delta = G.TFI(borders)
...
def oneovern(array, N, add=1):
    """Take one over N points from mesh.
    Usage: oneovern(array, (Ni,Nj,Nk))"""
    if isinstance(array[0], list): 
        b = []
        for i in array:
        ...
        return transform.oneovern(array, N, add)
...
def reorder(array, order):
    ...
    if isinstance(array[0], list): 
        ...
        return transform.reorder(array, order)     
...
def makeCartesianXYZ(array):
    if isinstance(array[0], list): 
        ...
        return makeCartesianXYZ__(array)   
...
def addkplane(array, N=1):
    """Add N k-plane(s) to a mesh.
    Usage: addkplane(array, N)"""
    if isinstance(array[0], list): 
        b = []
        for i in array:
            c = addkplane__(i, N)
...
def splitCurvatureRadius(array, Rs=100.):
    """Return the indices of the array where the curvature radius is low.
    Usage: splitCurvatureRadius(array, Rs)"""
    if isinstance(array[0], list):
        out = []
        for i in array:
            out += splitCurvatureRadius__(i, Rs)
        return out
    else: return splitCurvatureRadius__(array, Rs)
    
def splitConnexity(array):
    """Split array into connex zones.
    Usage: splitConnexity(array)"""
    if isinstance(array[0], list):
        out = []
        for i in array:
            out += transform.splitConnexity(i)
        return out
    return transform.splitConnexity(array)

def breakElements(array):
    """Break an array (in general NGON) in a set of arrays 
    of BAR, TRI, ... elements
    Usage: breakElements(array)"""
    if isinstance(array[0], list):
        b = []
        for i in array:
        ...
        if (len(array) == 4 and array[3] == 'NGON'):
            return transform.breakElements(array)
        else:
            return [array]
