...
def rotate(a, center, rotvect, angle=None, vectors=[]):
    ...
    return C.TZA(a, 'both', 'both',
                 ...
                 center, rotvect, angle, vectorsN,
                 center, rotvect, angle, vectorsC)

def _rotate(a, center, rotvect, angle, vectors):
    vectorsN = []; vectorsC = []
    for vect in vectors:
        if len(vect) == 3:
            loc = 0; vectname=[]
...
def deform(t, vector):
    ...
    Usage: deform(t, vector)"""
    tp = Internal.copyRef(t)
    _deform(tp, vector)
    return tp

def _deform(t, vector):
    if (len(vector) != 3):
    ...
    for z in nodes:
        ...
        if fa != []:
            ...
            print "Warning: deform: variables not found in zone."
            a = []
        if (a != []):
            coord = C.getFields(Internal.__GridCoordinates__, z)[0]
            coordp = Transform.deform(coord, a)
            C.setFields([coordp], z, 'nodes')
    return None
...
def deformMesh(a, surfDelta, beta=4.):
    ...
    return C.TZA(a, 'nodes', 'nodes', Transform.deformMesh, None, info, beta)

def _deformMesh(a, surfDelta, beta=4.):
    info = C.getAllFields(surfDelta, 'nodes')
    return C._TZA(a, 'nodes', 'nodes', Transform.deformMesh, None, info, beta)
...
def subzoneStruct__(t, minIndex, maxIndex):
    ...
    for z2 in nodes2:
        ...
        z2 = subzoneGC__(z2, dim, imin, imax, jmin, jmax, kmin, kmax, \
                         dimt0[4], dimt0[1], dimt0[2], dimt0[3], ni0, nj0, nk0)
        noz += 1
        if parent is not None: parent[2][nb] = z2
        else: t2 = z2
...
def splitNParts__(zones, N, multigrid, dirs):
...
def splitNParts(t, N, multigrid=0, dirs=[1,2,3]):
    ...
    allZones = splitNParts__(zones, N, multigrid, dirs)
