...
def getCGNSkeys(key, verbose=True):
    """Return the CGNS key, if exists, of the elsA key.
...
def _addFlowSolution(t, name='', loc='CellCenter', variables=None,
    ...
    for zone in Internal.getZones(t):
        ...
        if addBCExtract:
            ...
            for bc in Internal.getNodesFromType(zone, "BC_t"):
                SubChild = Internal.createNode(bc[0], "UserDefinedData_t")
                Internal.createChild(SubChild, "PointRange", "DataArray_t",Internal.getNodesFromName(bc, "PointRange")[0][1])
...
def addFlowSolutionEoR(t, name='', variables=None, governingEquations=None,
    ...
    """
    tp = Internal.copyRef(t)
    addFlowSolutionEoR_(tp, name, variables, governingEquations,
                        writingFrame, addBCExtract, protocol)
    return tp

def addFlowSolutionEoR_(t, name='', variables=None, governingEquations=None,
                        writingFrame='relative', addBCExtract=False,
                        protocol="end"):
...
def rmGCOverlap__(t):
    ...
    for z in zgc:
      ...
      for node in gc:
        ...
        if gct != None: Internal._rmNode(z,node)
    return tp
...
#==============================================================================
def buildMaskFiles__(t, maskInTree=True, keepOversetHoles=True):
    tp = Internal.copyRef(t)
