...
#pragma omp for
        for (E_Int i = 0; i < ni; i++)
        {
          ...
          ind3 = i+j*ni+kp*nij;
          ind4 = i+jp*ni+kp*nij;
          cx[ind] = inv*(xp[ind1]+xp[ind2]+xp[ind3]+xp[ind4]);
          cy[ind] = inv*(yp[ind1]+yp[ind2]+yp[ind3]+yp[ind4]);
          cz[ind] = inv*(zp[ind1]+zp[ind2]+zp[ind3]+zp[ind4]);
        }
    // interface en j
    if (nj > 1)
    {
...
#pragma omp parallel default(shared)
    {
    E_Int ind,ip,jp,kp, ind1,ind2,ind3,ind4,ind5,ind6,ind7,ind8;
    for (E_Int k = 0; k < nk1; k++)
      for (E_Int j = 0; j < nj1; j++)
#pragma omp for
        for (E_Int i = 0; i < ni1; i++)
        {
          ...
          ind2 = ip + j*nil + k*nij;
          ind3 = i + jp*nil + k*nij;
          ind4 = ip + jp*nil + k*nij;
          ind5 = i + j*nil + kp*nij;
          ind6 = ip + j*nil + kp*nij;
          ind7 = i + jp*nil + kp*nij;
          ind8 = ip + jp*nil + kp*nij;
          cx[ind] = 1./8.*(xp[ind1]+xp[ind2]+xp[ind3]+
                           xp[ind4]+xp[ind5]+xp[ind6]+
                           xp[ind7]+xp[ind8]);
          cy[ind] = 1./8.*(yp[ind1]+yp[ind2]+yp[ind3]+
                           yp[ind4]+yp[ind5]+yp[ind6]+
                           yp[ind7]+yp[ind8]);
          cz[ind] = 1./8.*(zp[ind1]+zp[ind2]+zp[ind3]+
                           zp[ind4]+zp[ind5]+zp[ind6]+
                           zp[ind7]+zp[ind8]);
...
#pragma omp for
    for (E_Int i = 0; i < nelts; i++)
    {
      ...
      cx[i] = cx[i] * inv; cy[i] = cy[i] * inv; cz[i] = cz[i] * inv; 
    }
    }
  }
