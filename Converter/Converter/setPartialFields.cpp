...
PyObject* K_CONVERTER::setPartialFieldsPT(PyObject* self, PyObject* args)
{
   ...
   PyObject* listIndicesO;
   E_Int loc;
   if (!PYPARSETUPLEI(args, "OOOl", "OOOi", &zone, &arrayF, &listIndicesO, &loc))
     return NULL;
...
PyObject* K_CONVERTER::_setPartialFields(PyObject* self, PyObject* args)
{
  ...
  PyObject* listIndicesO;
  E_Int loc;
  if (!PYPARSETUPLEI(args, "OOOl", "OOOi", &zone, &listNumFields, &listIndicesO, &loc)) return NULL;
  ...
  for (E_Int v = 0; v < nfld; v++)
  {
    ...
    if (oneField->getSize() == 0 || resf == 0) isEmpty[v] = 1;
    else isEmpty[v] = 0;
