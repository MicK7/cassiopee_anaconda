...
# -- printTree
# print a tree to file or screen
def printTree(t, stdOut=None, file=None, gvim=False):
  Internal.printTree(t, stdOut, file, gvim)
...
def getNobNozOfZone(a, topTree):
   """Return the (nob, noz) of a in topTree.
    Usage: getNobNozOfZone(a, topTree)"""
   nob = 0; noz = 0
   for b in topTree[2]:
     noz = 0
     for z in b[2]:
...
# IN: b: base
# IN: toptree: le toptree de a
# OUT: nob: no de la base dans le toptree
# If base can not be found, return -1
# Must be faster than getParentOfNode
def getNobOfBase(base, topTree):
  """Return the nob of a base in topTree.
  Usage: getNobOfBase(base, topTree)"""
  nob = 0
  for b in topTree[2]:
...
def _setPartialFields1(t, listFields, listIndices, loc='nodes'):
  ...
  nzones = len(nodes)
  for c in xrange(nzones):
    Converter.converter._setPartialFields(nodes[c], listFields[c], listIndices[c], locI)
  return None
...
# -- convertArray2NGon
def convertArray2NGon(t):
  """Convert a zone to a NGON zone.
  Usage: convertArray2NGon(t)"""
  tp = Internal.copyRef(t)
  _convertArray2NGon(tp)
  return tp

def _convertArray2NGon(t):
  zones = Internal.getZones(t)
  gbcs = [] # geom BCs
  for z in zones:
...
def pushBC(t1, t2, type='F', tol=1.e-12):
  """Put BCs of t1 to t2.
...
def _rmBCOfName(t, bndName):
  ...
  if (len(names) == 1): # real bnd name
    ...
    for z in zones:
      ...
      for i in nodes:
        if (i[3] == 'BC_t' or i[3] == 'GridConnectivity1to1_t' or
            i[3] == 'GridConnectivity_t'):
          (parent, d) = Internal.getParentOfNode(z, i)
          del parent[2][d]
...
def getEmptyBCForZone__(z, pbDim, splitFactor):
  dims = Internal.getZoneDim(z)
  if dims[0] == 'Unstructured': # unstructured
    return getEmptyBCForUnstructZone__(z, dims, pbDim, splitFactor)
  else: return getEmptyBCForStructZone__(z, dims, pbDim, splitFactor)
...
def getEmptyBCForStructZone__(z, dims, pbDim, splitFactor):
  ...
  for bc in bnds:
    range = Internal.getNodeFromName1(bc, 'PointRange')
    if range is not None: wins.append(Internal.range2Window(range[1]))
  # BC match
  bnds = Internal.getNodesFromType2(z, 'GridConnectivity1to1_t')
  for bc in bnds:
    range = Internal.getNodeFromName1(bc, 'PointRange')
...
# -- Detect empty boundary conditions for unstruct zones
# Renvoie une liste des indices des faces des BC empty
def getEmptyBCForUnstructZone__(z, dims, pbDim, splitFactor):
  ...
  if defined != []:
    ...
    #defined = convertArray2NGon(defined)
    #defined = T.join(defined)
    #id0 = identifyElements(hook, defined)
    #tag[id0[:]-1] = 0
    for d in defined:
...
def tagWithFamily(z, familyName):
  """Tag a zone node or a BC node with a familyName.
  Usage: tagWithFamily(z, familyName)"""
  a = Internal.copyRef(z)
  _tagWithFamily(a, familyName)
  return a

def _tagWithFamily(a, familyName):
  res = Internal.getNodesFromName(a, familyName)
  if res == []:
...
def getFamilyZones(t, familyName):
  ...
  zones = Internal.getZones(t)
  for z in zones:
    res = Internal.getNodesFromName1(z, 'FamilyName')
    for i in res:
...
def getFamilyBCs(t, familyName):
  ...
  for z in zones:
    ...
    nodes += Internal.getNodesFromType2(z, 'GridConnectivity1to1_t')
    for n in nodes:
      res = Internal.getNodesFromName(n, 'FamilyName')
      for i in res:
        val = Internal.getValue(i)
        val = val.strip()
        for f in families:
...
# -- getFamilyBCNamesOfType ( wild cards possible)
def getFamilyBCNamesOfType(t, bndType=None):
  ...
  out = []
  nodes = Internal.getNodesFromType3(t, 'FamilyBC_t')
  for n in nodes:
    type = Internal.getValue(n)
    if bndType is None:
      (p, c) = Internal.getParentOfNode(t, n)
      if p[3] == 'Family_t': out.append(p[0])
    elif (type == bndType):
      (p, c) = Internal.getParentOfNode(t, n)
      if p[3] == 'Family_t': out.append(p[0])
  # unique the family name (if define in more than one base)
  seen = set()
  seen_add = seen.add
...
def getFamilyZoneNames(t):
  ...
  for n in nodes:
    ret = Internal.getNodesFromType(n, 'FamilyBC_t')
    if ret == []: out.append(n[0])
  # unique the family name (if define in more than one base)
  seen = set()
  seen_add = seen.add
  out = [x for x in out if x not in seen and not seen_add(x)]
      ...
      for i in xrange(len(la)):
        ...
        for z in zones:
            ...
            if (dim == 'Unstructured'):
              ...
              try: import Transform; import Generator
              except: pass
              else:
...
def center2Node(t, var='', cellNType=0):
...
def addState2Node__(a, state, value):
  ...
  if (state == 'EquationDimension' or state == 'GoverningEquations'
      or state == 'TurbulenceModel'):
    H = []
    for n in a[2]:
    ...
    for b in BCs:
      ...
      if p != []:
        ...
        if (n != [] and loc == 'FaceCenter'):
          if type == 'FamilySpecified':
            BCFZ += [name, n[0][1]]
          else: BCFZ += [name+Internal.SEP2+type, n[0][1]]
    BCFaces.append(BCFZ)
  return BCFaces
...
def _addBCFaces(t, BCFaces):
  ...
  for z in zones:
    ...
    for i in xrange(l/2):
      ...
      if len(names) == 2: name = names[0]; type = names[1]
      else: name = names[0]; type = 'FamilySpecified:'+names[0]
      faces = myBC[2*i+1]
      _addBC2Zone(z, name, type, faceList=faces)
    c += 1
  return None
...
def convertPyTree2FFD(zone,RefStat,FlowEq,nd):
  ...
  print "FlowEq =", FlowEq
  Converter.converter.convertPyTree2FFD(zone,RefStat,FlowEq,nd,Internal.__FlowSolutionNodes__,Internal.__FlowSolutionCenters__ )
  return None
