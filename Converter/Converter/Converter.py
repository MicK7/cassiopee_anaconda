...
def randomizeVar(array, var, deltamin, deltamax):
    """Randomize a field defined by var within a range [a-deltmin, a+deltamax]
...
def getValue(array, ind):
    ...
    if isinstance(ind, tuple):
        ...
        if len(ind) == 3:
            ...
            raise ValueError("getValue: too much values in index tuple.")
    else:
        index = ind
...
def setValue(array, ind, field):
    ...
    nf = ar.shape[0]
    nf2 = len(field)
    if nf2 != nf: raise ValueError("setValue: field is badly dimensioned.")
    for i in xrange(nf): ar[i, index] = field[i]
    return None
...
def node2Center(array):
    """Convert array defined on nodes to array defined on centers.
    Usage: node2Center(array)"""
    if isinstance(array[0], list):
        b = []
        for i in array:
            b.append(converter.node2Center(i))
        return b
    else:
        return converter.node2Center(array)
...
#==============================================================================
def getVarNames(array):
    """Get variable names.
    Usage: getVarNames(array)"""
    if isinstance(array[0], list):
        allvars = []
        for i in array:
            v = i[0].split(",")
            allvars.append(v)
    else: allvars = array[0].split(",")
    return allvars
...
def identifyElements(hook, a, tol=1.e-11):
    ...
    if isinstance(a[0], list):
        b = []
        for i in a:
            b.append(converter.identifyElements(hook, convertArray2NGon(i), tol))
        return b
    else:
        return converter.identifyElements(hook, convertArray2NGon(a), tol)
...
def nearestElements(hook, a):
    ...
    if isinstance(a[0], list):
        b = []
        for i in a:
            b.append(converter.nearestElements(hook, convertArray2NGon(i)))
        return b
    else:
        return converter.nearestElements(hook, convertArray2NGon(a))
