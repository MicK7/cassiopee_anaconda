...
#==============================================================================
def readZones(t, fileName, format=None, proc=None, zoneNames=None):
    tp = Internal.copyRef(t)
    _readZones(tp, fileName, format, proc, zoneNames)
    return tp

def _readZones(t, fileName, format=None, proc=None, zoneNames=None):
  if (zoneNames is None and proc is None): return None
  bases = Internal.getBases(t)
  if proc is not None: # load zones by proc
      ...
      for b in bases:
          ...
          for z in zones:
              ...
              if nproc is not None:
                  ...
                  if (nproc == proc): paths.append('/'+b[0]+'/'+z[0])
  ...
  for p in paths:
      ...
      if z is not None:
          if proc is not None: # recopie les solveurs data
              nproc = Internal.getNodeFromName2(z, 'proc')
              if nproc is not None:
                  nproc = Internal.getValue(nproc)
              else: nproc = 0
