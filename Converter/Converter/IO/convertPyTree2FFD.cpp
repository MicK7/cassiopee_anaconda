...
PyObject* K_CONVERTER::convertPyTree2FFD(PyObject* self, PyObject* args)
{
  ...
  PyObject* RefStat;
  PyObject* FlowEq;
  char* contnodes=NULL; char* contcenters=NULL; char* GridCoordinates=NULL;
  E_Int nd , ii;
  E_Int neq = 5 ;

  if (!PYPARSETUPLEI(args, "OOOlss", "OOOiss", 
                     &zone, &RefStat, &FlowEq, &nd, &contnodes, &contcenters)) return NULL;
...
//  
  E_Int res = K_PYTREE::getFromZone(zone, 1, loc, varString,
                        ...
                        cn, cnSize, cnNfld, eltType, hook, GridCoordinates, contnodes, contcenters)  ;
   if( test == 1 ){
...
//     cerr <<"c'est bon ....."<<endl ;
   // extraction des variables
    ...
    E_Float* TurbulentDistance;
    E_Float MutsMu[nelmt];
           ...
           posVEK = K_ARRAY::isNamePresent("ViscosityEddyKinetic", varString); //Nut
            if( posVM == -1 && posVK == -1 )
             ...
             if ( posVM != -1 && posVE != -1) {
                  ...
                  K_CONVERTER::calculateMutsMu(nelmt,MutsMu,Mut,Mu);
               }
                  ...
                  E_Float* Mut = fields[posVEK] ;
                  E_Float* Mu  = fields[posVM] ;
                  K_CONVERTER::calculateMutsMu(nelmt,MutsMu,Mut,Mu,Density);  //MutsMu= Ro*Vek/vm
               }
             else if ( posVE != -1 && posVK != -1) {
                  E_Float* Mut = fields[posVEK] ;
                  E_Float* Mu  = fields[posVM] ;
                  K_CONVERTER::calculateMutsMu(nelmt,MutsMu,Mut,Mu,NULL,Density); //MutsMu= Ve/(Ro*Vk)
               }
             else
               {             
              PyErr_SetString(PyExc_TypeError,
...
//   E_Float Energy_l[nlimt]; E_Float MutsMu_l[nlimt];
   E_Float Var_l[neq*nlimt];
   K_CONVERTER::getVarBC(zone, Var_l, ielmtmtch2, &nlimt) ;
//
...
// cnNfld est > npoint 
    E_Int nodmtch[NElCon] ; E_Int kpoinmtch[NElCon] ;
    cerr << "appel de splitElementConnectivity"<<endl ;
    K_CONVERTER::splitElementConnectivity(zone, &npoint, nodmtch, kpoinmtch) ;
//
    E_Int izoneznzn [nlimt] ; E_Int ielmtznzn [nlimt];
...
// kod_int =(kodcc, kodsolver,kodnst,kod2d)
    writeffdfilef_(nd, kod_int, nnode, nelmt,
		   ...
		   nlimt, Var_l,
                   nmtch, ielmtmtch1, ielmtmtch2, izoneznzn, ielmtznzn,
