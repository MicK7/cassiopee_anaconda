                                     ...
                                     hid_t sid)
{
  ...
  r = (PyArrayObject*)PyArray_EMPTY(dim, npy_dim_vals, NPY_INT, 1);
                                     ...
                                     hid_t sid)
{
  ...
  r = (PyArrayObject*)PyArray_EMPTY(dim, npy_dim_vals, NPY_DOUBLE, 1);
                                            ...
                                            PyObject* data)
{
  ...
  if (data == NULL)
  {
    ...
    // for (E_Int nn = 0; nn < dim; nn++) printf("getArrayR8 of dims :: %d \n", dims[nn]);
    for (E_Int nn = 0; nn < dim; nn++) npy_dim_vals[nn] = dims[nn];
    data = PyArray_EMPTY(dim, npy_dim_vals, NPYtype, 1);
  }
...
// path: /Base/cart12
//==========================================================================
hid_t K_IO::GenIO::openGroupWithLinks(hid_t start, char* path)
{
...
PyObject* K_IO::GenIO::hdfcgnsReadFromPaths(char* file, PyObject* paths)
{
  ...
  for (E_Int i = 0; i < size; i++)
  {
    ...
    hid_t gid = openGroupWithLinks(fid, path);
    if (gid < 0)
    { 
