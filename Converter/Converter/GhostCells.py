...
def fillJoinGhostCellsStruct__(zp, join, modified, joininfo,
    ...
    #print zp[0], zdonor[0], joininfo[1][0][1], joininfo[2][0][1]
    prange = joininfo[1]
    ...
    for v in ['Velocity','Momentum']:
        ...
        vectname = ['centers:'+v+'X','centers:'+v+'Y','centers:'+v+'Z']
        vectorsR.append(vectname)
    ...
    if (translVect != [] and rotationData == []): # translation seulement
        ...
        if fieldsn != []:
            fieldsn = Transform.rotate(fieldsn, (xc,yc,zc), (vx,vy,vz), -angle, vectors=vectorsR) 
            PyTree.setFields([fieldsn],zdonor, 'nodes',writeDim=False)
        if fieldsc != []:
            fieldsc = Transform.rotate(fieldsc, (xc,yc,zc), (vx,vy,vz), -angle, vectors=vectorsR) 
...
def  getBCDataSetContainers(name,z):
    ...
    if dims[0] == 'Unstructured': 
        print 'addGhostCells: getBCDataSetContainers not yet implemented for unstructured zones.'
        return None

    if name == Internal.__GridCoordinates__: return None

    # Extract BCDataSet info : loc, bc range, fields
    allBCDatas=[]; allRanges=[]; allLocDS=[]
    for bcNode in Internal.getNodesFromType3(z,"BC_t"):
        res = Internal.getBCDataSet(z, bcNode,withLoc=True) # numpy of fields at bcs + loc
        if res is not None:
            datas,locds = res
            w = Internal.getNodeFromName2(bcNode, 'PointRange')
            w = Internal.range2Window(w[1]) # imin,imax,jmin,jmax,kmin,kmax
    ...
    if (name  == Internal.__FlowSolutionNodes__) or (name == Internal.__FlowSolutionCenters__):
        loc = 'Vertex'
        f = Internal.getNodeFromName2(z, name)
        if f is not None:
            # location of flow solution
            l = Internal.getNodeFromType1(f, 'GridLocation_t')
            if (l is not None): loc = Internal.getValue(l)
            else:
                if (name == Internal.__FlowSolutionNodes__): loc = 'Vertex'
                else: loc = 'CellCenter'
        ...
        for v in name:
            ...
            if (len(varname) == 2): # center value
                if (varname[0] == 'centers'): loc = 'CellCenter'; container = Internal.__FlowSolutionCenters__
                varname = varname[1]
            else: varname = varname[0] # node value

            # search vars in nodes/centers container
            containerNode = Internal.getNodeFromName1(z, container)
            if containerNode is None: return None
            else: # location of flow solution
                l = 'Vertex'
                l = Internal.getNodeFromType1(containerNode, 'GridLocation_t')
                if (l is not None): ploc = Internal.getValue(l)
                else:
                    if (name == Internal.__FlowSolutionNodes__): ploc = 'Vertex'
                    else: ploc = 'CellCenter'
            if loc == ploc: # variable is consistent with the container FlowSolution
                if loc == 'Vertex':  matchingLoc = 'Vertex'
                else: matchingLoc = 'FaceCenter' 
                f0 = Internal.getNodeFromName1(containerNode,varname)
                if f0 is not None:
                    if f0[3]=='DataArray_t':
                        dataSetL = [] # [[BCRange1,fieldBC],[BCRange2,fieldBC2],[...],...]
                        for nobcdata in xrange(nbcdataset):
                            if allLocDS[nobcdata]==matchingLoc: # OK: locations are consistent
                                datas = Internal.getNodesFromName1(allBCDatas[nobcdata],varname) 
                                if datas != []:
                                    dataSetL+=[[allRanges[nobcdata], datas[0][1]]]
                        if dataSetL!=[]:# variable exists both in flow solution and in bcdataset
                            dataFS = [f0[1], loc, dataSetL]
...
def _setBCDataInGhostCellsStruct__(z, dim, modified, d):
    ...
    imz = dim[1] ; jmz = dim[2] ; kmz = dim[3]
    for name in modified:
        containers = getBCDataSetContainers(name,z) 
        if containers is not None:
            for cont in containers:
                ...
                Converter.converter._setBCDataInGhostCellsStruct(dataFS[0], bcranges, dataBC, imz, jmz, kmz, d, locI)
    return None
