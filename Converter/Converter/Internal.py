...
def _setName(node, name=None):
    setName(node, name)
    return None

# -- setType
# Set node type (modify node)
def setType(node, ntype=None):
...
def _setType(node, ntype=None):
    setType(node, ntype)
    return None

# -- createNode
# create a node
def createNode(name, ntype, value=None, children=None):
...
def newElements(name='Elements', etype='UserDefined',
    ...
    if parent is None:
        node = createNode(name, 'Element_t', value=[etp,eboundary])
    else: node = createUniqueChild(parent, name, 'Element_t',
                                   value=[etp,eboundary])
...
def newBC(name='BC', pointRange=None, pointList=None,
    ...
    if family is not None:
        createChild(node, 'FamilyName', 'FamilyBC_t', value=family)
    return node
...
def newChemicalKineticsModel(value='Null', parent=None):
    ChemicalKinecticsModel_l = ['Null', 'UserDefined', 'Frozen',
...
# OUT: le chemin du noeud
# Note: tres cher
def getPath(t, node):
    if t is node: return []
    else: pathList = [node[0]]
    getPath__(t, node, pathList)
    spath = ''
    for i in pathList: spath += i + '/'
    return spath[:-1]

def getPath__(t, node, pathList):
    (parentNode, c) = getParentOfNode(t, node)
    if parentNode is not None:
        pathList.insert(0, parentNode[0])
        getPath__(t, parentNode, pathList)

# -- Retourne un noeud d'apres son path
# IN: node: noeud de depart pour la recherche
# IN: path: chemin relatif par rapport a ce noeud
def getNodeFromPath(node, path):
    if path == '' or path == '/': return node
    if path[0] == '/': path = path[1:]
    if path[-1] == '/': path = path[:-1]
    if node[0] == path: return node
    if node[3] == 'CGNSTree_t': p = path.replace(node[0]+'/','')
    else: p = path
    p = p.split('/')
    ...
    else: return getNodeFromPath__(node, p)
...
def getNodesFromType(node, ntype):
    result = []
    isStd = isStdNode(node)
    if isStd >= 0:
        for c in node[isStd:]: getNodesFromType__(c, ntype, result)
...
def getByType(node, ntype, recursive=-1):
    if recursive == 1: ret = getNodesFromType1(node, ntype)
    elif recursive == 2: ret = getNodesFromType2(node, ntype)
    elif recursive == 3: ret = getNodesFromType3(node, ntype)
    else: ret = getNodesFromType(node, ntype)
    return [ntype, None, ret, None]

# -- Retourne un seul noeud (no wildcard) - Fast
def getNodeFromType(node, ntype):
    isStd = isStdNode(node)
    if isStd >= 0:
        for c in node[isStd:]:
        ...
        return None
    else: return getNodeFromType__(node, ntype)
...
def getPathsFromType__(node, ntype, current, result):
    if node[3] == ntype: result.append(current+'/'+node[0])
...
# Parcours tout l'arbre. Widcards possibles.
def getNodesFromName(node, name):
    result = []
    isStd = isStdNode(node)
    if isStd >= 0:
        if ('*' in name)|('?' in name)|('[' in name):
            for c in node[isStd:]: getNodesFromName___(c, name, result)
        else:
            for c in node[isStd:]: getNodesFromName__(c, name, result)
    else:
        if ('*' in name)|('?' in name)|('[' in name):
            getNodesFromName___(node, name, result)
        else: getNodesFromName__(node, name, result)
    return result
...
def getByName(node, name, recursive=-1):
    if recursive == 1: ret = getNodesFromName1(node, name)
    elif recursive == 2: ret = getNodesFromName2(node, name)
    elif recursive == 3: ret = getNodesFromName3(node, name)
    else: ret = getNodesFromName(node, name)
    return [name, None, ret, None]

# -- Retourne un seul noeud (no wildcard) - Fast
def getNodeFromName(node, name):
    isStd = isStdNode(node)
    if isStd >= 0:
        for c in node[isStd:]:
            ...
            if ret is not None: return ret
        return None
    else: return getNodeFromName__(node, name)
...
# On demarre le parcours a partir de node.
def getNodesFromValue(node, value):
    result = []
    isStd = isStdNode(node)
    if isStd >= 0:
        for c in node[isStd:]: getNodesFromValue__(c, value, result)
    else: getNodesFromValue__(node, value, result)
    return result
...
def getParentOfNode(start, node):
    idNode = id(node)
    isStd = isStdNode(start)
    if isStd >= 0:
        d = isStd
        for c in start[isStd:]:
            if (id(c) == idNode): return (start, d)
            d += 1
        r = None; d = 0
        for c in start[isStd:]:
        ...
        return (None, 0)
    else: return getParentOfNode__(start, idNode)
...
def repr__(t, Out=None, DEB='', LAST=False):
    ...
    if (len(t[2]) < 2): out += "%d son],'%s']"%(len(t[2]),t[3])
    else: out += "%d sons],'%s']"%(len(t[2]),t[3])
    ...
    for ichild in range(len(t[2])):
        if ichild == (len(t[2])-1):
            repr__(t[2][ichild], Out, outDEB, LAST=True)
        else:
            repr__(t[2][ichild], Out, outDEB)
    if First: return Out

def printTree(t, stdOut=None, file=None, gvim=False):
    import sys
    stdNode = isStdNode(t)
    if (stdNode == -2): return
    elif (stdNode == -1): rep = ''.join(repr__(t))
    else:
        out = []
        for b in t: out += repr__(b)
        rep = ''.join(out)
    if stdOut: sys.stdout.write(rep)
    if gvim:
        fd, tmpfl = mkstemp('.py')
        os.write(fd, rep)
        os.close(fd)
        os.system('gvim -f %s ;rm -f %s' %(tmpfl, tmpfl))
    if file:
        fi = open(file, 'w')
        fi.write(rep)
        fi.close()
    if (file is None and not(gvim) and stdOut is None):
...
def convertDataNodes2Array2(nodes, dim, connects, loc=-1):
    ...
    if eltType == 22: # NGON
        info = connect1
        e = Internal.getNodeFromName(connect1, 'ElementConnectivity')
        f = Internal.getNodeFromName(connect2, 'ElementConnectivity')
        g = Internal.getNodeFromName(connect1, 'FaceIndex')
        h = Internal.getNodeFromName(connect1, 'ElementIndex')
        cr = [e, f, g, h]
    else:
        e = Internal.getNodeFromName(connect, 'ElementConnectivity')
        cr = [e[1]]
...
def groupBCByBCType(t, type='BCWall', name='FamWall'):
    a = copyRef(t)
    _groupBCByBCType(a,type,name)
    return a

def _groupBCByBCType(t, type='BCWall', name='FamWall'):
    for base in getBases(t):
        ...
        if found:
            bcvalue=createNode('FamilyBC','FamilyBC_t',value=type)
...
def setElementConnectivity2(z, array):
  ...
  if GENodes == []:
      ...
      if etype != 22 and etype != 23: # Elements->Nodes connectivities
          ...
          Internal.createUniqueChild(info2, 'FaceIndex', 'DataArray_t', array[2][2])
          # Creation du noeud NFACE_n : connectivite Elements->Faces
          etype, stype = eltName2EltNo('NFACE')
          i2 = numpy.empty((2), numpy.int32); i2[0] = etype; i2[1] = 0
          info.append(['NFaceElements', i2, [], 'Elements_t'])
          ...
          info2.append(['ElementConnectivity', array[2][1], [], 'DataArray_t'])
          # Tableau ElementIndex (PH)
          Internal.createUniqueChild(info2, 'ElementIndex', 'DataArray_t', array[2][3])
          _updateElementRange(z)
...
def _fixNGon(t, remove=False, breakBE=True, convertMIXED=True, addNFace=True):
    ...
    for z in zones:
        ...
        if stype == 2: # Unstructured
            ...
            # Reorder: NGON en premier si BE
            if (BE > 0 and NGON >= 0):
                ngon = sons[NGON]
                n = getNodeFromName1(ngon, 'ElementRange')[1]
                shiftBC = n[0]
                #print "Shift BC", shiftBC
                # Remet les BCs d'aplomb
                ...
                for b in joins:
                    ...
                    for p in pl:
                        ...
                        if p[0] == 'PointListDonor':
                            zdonor = getValue(b)
                            pln = p[1]
                            pln[:] = pln[:]-shift[zdonor]+1

                            # DBX
                            #faces = pln
                            #zd = getNodeFromName2(t, zdonor)
                            #r = getElementRange(zd, type="NGON")

                            #faces1 = faces[faces > r[1]-r[0]+1]
                            #faces2 = faces[faces < 1]
                            #print 'checking ',b[0],z[0],zdonor,r[1]-r[0]+1
            ...
            for b in joins:
                ...
                if pld is not None:
                    ...
                    if dictOfZTypes[zdonorname] == 2: # hybrid join
                        pln = pld[1]
                        pln[:] = pln[:]-shift[zdonorname]+1
...
def _createElsaHybrid(t, method=0, axe2D=0):
    ...
    for z in zones:
         ...
         if found:
             node = GEl[NGON]
             CE = getNodeFromName1(node, 'ElementConnectivity')
             PE = getNodeFromName1(node, 'ParentElements')
             if PE is None:
                 _adaptNFace2PE(z, remove=False)
                 PE = getNodeFromName1(node, 'ParentElements')
             er = getNodeFromName1(node, 'ElementRange')
             nfaces = er[1][1]-er[1][0]+1
             child = createUniqueChild(z, ':elsA#Hybrid', 'UserDefinedData_t')
             # to be removed
             sct = numpy.arange((nfaces), dtype=numpy.int32)
             newDataArray('SortedCrossTable', value=sct, parent=child)
             inct = numpy.empty((nfaces), dtype=numpy.int32)
             newDataArray('IndexNGONCrossTable', value=inct, parent=child)
             # OK
             ict = numpy.empty((nfaces), dtype=numpy.int32)
             bcct = numpy.empty((nfaces), dtype=numpy.int32)
             newDataArray('InversedCrossTable', value=ict, parent=child)
             newDataArray('BCCrossTable', value=bcct, parent=child)
             if axe2D > 0:
                 x = getNodeFromName2(z, 'CoordinateX')[1]
                 y = getNodeFromName2(z, 'CoordinateY')[1]
                 z = getNodeFromName2(z, 'CoordinateZ')[1]
             else: x = None; y = None; z = None
             (iTRI, iQUADS, eTRI, eQUADS) = converter.createElsaHybrid(
                 CE[1], PE[1], ict, bcct, inct, method,
                 axe2D, x, y, z)
             if method == 0:
                 newDataArray('InternalTris', iTRI, parent=child)
                 newDataArray('InternalQuads', iQUADS, parent=child)
                 newDataArray('ExternalTris', eTRI, parent=child)
                 newDataArray('ExternalQuads', eQUADS, parent=child)
             else:
                 newDataArray('InternalElts', iTRI, parent=child)
                 newDataArray('ExternalElts', eTRI, parent=child)
         else:
