...
def nurbs(t, weight='weight', order=3, N=100, M=100, density=-1):
    ...
    Usage: nurbs(ctrlsPts, order, N)"""
    w = C.getFields(Internal.__FlowSolutionNodes__, t)[0]
    Pts = C.getFields(Internal.__GridCoordinates__, t)[0]
    ws = []; ws.append(weight)
    w = Converter.extractVars(w, ws)
    surf = Geom.nurbs(Pts, w, order, N, M, density)
...
def connect1D(curves, sharpness=0, N=10, lengthFactor=1.):
    ...
    z = Geom.connect1D(a, N, lengthFactor)
    return C.convertArrays2ZoneNode('joined', [z])
