                                          ...
                                          PyObject* args )
{
  ...
  FldArrayF coordCC1; 
  FldArrayF coordCC2;
  compCCCenters( coord1, *cn1, coordCC1);
  compCCCenters( coord2, *cn2, coordCC2);
  ...
  compTetraType1(1, coordpoly1, *cnpoly1, 
                 *cn1, coordCC1, coord1, coord2, 
                 nv, ne, type1, coord, cn, type);

  compTetraType1(2, coordpoly2, *cnpoly2, 
  ...
  compTetraType12(coordpoly1, *cnpoly1, 
                  *cn1, coordCC1, coord1, 
                  coordpoly2, *cnpoly2,
...
*/
//===========================================================================
void K_GEOM::compCCCenters(FldArrayF& coord, FldArrayI& cn, FldArrayF& coordCC)
{
  E_Int nelts = cn.getSize();
                            ...
                            FldArrayI& type)
{
  ...
  K_COMPGEOM::boundingBox(1, 2, 3, coordpoly1, 
  ...
  for (E_Int et = 0; et < ne1; et++) //parcours des triangles
  {
    ...
    E_Int ind = findNearestVertex( coord2, P);
...
//=============================================================================
// Parmi tous les points de coord, trouve celui qui est le plus proche 
// de P
//=============================================================================
E_Int K_GEOM::findNearestVertex(FldArrayF& coord, E_Float* P)
{
  E_Int np = coord.getSize();
  E_Int ind = -1;
  E_Float dmax = E_MAX_FLOAT;
  E_Float d, dx, dy, dz;
...
void K_GEOM::compVoronoiEdges(E_Int nvertex, FldArrayI& cn, FldArrayI& vedges)
{
  ...
  vector< vector<E_Int> > cEEN(nelts);
  K_CONNECT::connectEV2EENbrs("TRI", nvertex, cn, cEEN);
...
//===========================================================================
void K_GEOM::compTetraType12(FldArrayF& coordpoly1, FldArrayI& cnpoly1,
                             FldArrayI& cn1,
                             FldArrayF& coordCC1, FldArrayF& coord1,
                             FldArrayF& coordpoly2, FldArrayI& cnpoly2,
                             ...
                             FldArrayF& coord, FldArrayI& cn, FldArrayI& type)
{
  ...
  K_COMPGEOM::boundingBox(1, 2, 3, coordpoly1, 
                          xmin1, ymin1,zmin1, xmax1, ymax1, zmax1);
  K_COMPGEOM::boundingBox(1, 2, 3, coordpoly2, 
  ...
  compVoronoiEdges(coordpoly1.getSize(), cn1, vedges1);

  // Calcul des aretes du diagramme de Voronoi de T2
  compVoronoiEdges(coordpoly2.getSize(), cn2, vedges2);
                                    ...
                                    FldArrayF& coordc, FldArrayI& cnc)
{
  ...
  E_Int isIn = K_COMPGEOM::pointInPolygon2D(coordc, cnc, P, eps, 
                                            NULL, NULL);
  if ( isIn == 0 ) return 0;
  
  P[0] = 0.5*(x3+x2); P[1] = 0.5*(y3+y2); P[2] = z;
  isIn = K_COMPGEOM::pointInPolygon2D(coordc, cnc, P, eps, 
                                      NULL, NULL);
  if ( isIn == 0 ) return 0;
