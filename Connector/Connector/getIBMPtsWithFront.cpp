...
PyObject* K_CONNECTOR::getIBMPtsWithFront(PyObject* self, PyObject* args)
{
...
#ifdef E_DOUBLEINT 
    if (!PyArg_ParseTuple(args, "OOOOll", &allCorrectedPts, &bodySurfaces, &frontSurfaces, &normalNames, &signOfDist, &depth))
#else
    if (!PyArg_ParseTuple(args, "OOOOii", &allCorrectedPts, &bodySurfaces, &frontSurfaces, &normalNames, &signOfDist, &depth))
#endif
        return NULL;
    ...
    for (E_Int noz = 0; noz < nzones; noz++)
    {
        ...
        for (E_Int ind = 0; ind < npts; ind++)
        {
            ...
            /*------------------------------------------------------------*/
            /*  STEP 1/ projection following normals (grad of distance  ) */
            /*------------------------------------------------------------*/
            valid = 1;
            //1. ProjectDir for pt onto the bodies
...
# include "IBC/getIBMPts_projectDirBodies.h"
            ...
            // 2. projectDir for pt onto front surfaces 
            dirx0 = ptrNX[ind];
            diry0 = ptrNY[ind];
            dirz0 = ptrNZ[ind];
            //nx = dirx0; ny = diry0; nz = dirz0; //normale orientee vers l'exterieur
...
#  include "IBC/getIBMPts_projectDirFront.h"
            ...
            else goto projectOrthoBody;

            /*--------------------------------------------------------------------------------------*/
            /* STEP2/ ortho projection on the bodies + new normals for projectDir on the front */
            /*--------------------------------------------------------------------------------------*/
...
# include "IBC/getIBMPts_projectOrthoBodies.h"
            ...
            dirx0 = sign*(xsav-xc0); //sign=1 (interieur): suit le vecteur CW
            diry0 = sign*(ysav-yc0); //sign=-1 (exterieur): suit le vecteur WC
            dirz0 = sign*(zsav-zc0);
...
#  include "IBC/getIBMPts_projectDirFront.h"
            ...
            if ( valid == 1 ) goto endofpt; // both pts are valid
            else goto projectOrthoFront;

            /*-----------------------------------------------------------------*/
            /* STEP3/ PROJECTION ORTHOGONALE SUR LE FRONT + DIR SUR L OBSTACLE */
            /*-----------------------------------------------------------------*/
            projectOrthoFront:;
