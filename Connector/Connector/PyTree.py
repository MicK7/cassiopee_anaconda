...
def _connectMatchHybrid__(a, tol, dim, glob):
    ...
    # Get exterior faces
    # allExtIndices = []
    # allExtFaces = P.exteriorFaces(zones, indices=allExtIndices)
    # print 'AVANT ', len(allExtFaces), len(allExtIndices)
    # print allExtFaces
...
def getInterpolationDomains__(bases, nob0, noz0, zn0, surfacest=[],
    ...
    try: import Generator as G; bb = 1
    except: bb = 0
    nbases = len(bases)
    names = []; zones = []; cellns = []
    for nob in xrange(nbases):
        if nob != nob0:
            ...
            for noz in xrange(len(nodes)):
                zn = C.getFields(Internal.__GridCoordinates__,nodes[noz])[0]; intersect = 1
                if (bb==1 and G.bboxIntersection(zn0, zn) == 0): intersect = 0
                if intersect == 1:
...
def getBBIntersectingDomains(A, B, tol=1.e-6):
    ...
    doms = []
    zones2 = Internal.getZones(B)
    for z2 in zones2:
        zones1 = Internal.getZones(A)
        for z1 in zones1:
            if (z1 != z2 and G.bboxIntersection(z1, z2, tol=tol) == 1):
                doms.append(z1)
    return doms
...
def getIntersectingDomains(t, t2=None, method='AABB', taabb=None, tobb=None,
    ...
    if method == 'AABB':
        ...
        for z1 in zonesNames:
            aabb1 = Internal.getNodeFromName2(taabb,z1)
            IntDict[z1] = []
            for z2 in zonesNames2:
                if z1 != z2:
                    aabb2 = Internal.getNodeFromName2(taabb2,z2)
        ...
        for z1 in zonesNames:
            obb1 = Internal.getNodeFromName2(tobb,z1)
            IntDict[z1] = []
            for z2 in zonesNames2:
                if z1 != z2:
                    obb2 = Internal.getNodeFromName2(tobb2,z2)
...
def _setIBCDataForZone__(z, zonesDnr, correctedPts, wallPts, interpPts, loc='nodes', \
    ...
    resInterp = Connector.setInterpData__(interpPts, arraysD, order, penalty, nature, method, hook, dim)
...
# IN: aD: arbre, zone, bases des donneurs
# IN: order: ordre des interpolations (2, 3, 5)
# IN: method='lagrange', 'leastsquares'
# IN: penalty=1: penalise une cellule donneuse en terme de volume si elle est au bord
# IN: nature=0: aucun sommet de la cellule d'interpolation ne doit etre avec un cellN=0
...
def setInterpData(tR, tD, double_wall=0, order=2, penalty=1, nature=0,
    ...
    # 2. Calcul des coefs et donneursInternal.getNodeFromName2(zones[noz1], name1)
    #---------------------------------------------------------------------------
    if hook is not None:
        allHooks = hook[:]
        allHooksL = allHooks[:]
    ...
    for z in zonesRcv:
        ...
        if cellNPresent != -1:
            ...
            if locR == 'nodes':
                ...
                nature = 0 # critere pour les faces : pas de pt masque ds la molecule donneuse
                an = C.getFields(Internal.__GridCoordinates__, z)[0]
                ac = Converter.node2Center(an)
                cellN = C.getField('centers:cellN',z)[0]
                ac = Converter.addVars([ac,cellN])
                if double_wall == 1 and interpWallPts[nozr] != []:# dw : liste d arrays
                    isdw = 1
                    EXPts = Connector.getEXPoints__(an, ac)
                    for nozd in xrange(nzonesDnr):
                        EXPts2 = Connector.changeWallEX__(EXPts, ac, an, interpWallPts[nozr], donorSurfs[nozd])
                        interpPts.append(EXPts2)
                else:  # pas de dw: un seul array
                    interpPts = Connector.getEXPoints__(an, ac)

            #---------------------------------------------
            # Etape 2 : calcul des donnees d'interpolation
            #---------------------------------------------
            # resInterp = [rcvInd1D,donorInd1D,donorType,coefs,extrap,orphan, EXdirs]
            resInterp = Connector.setInterpData__(interpPts, arraysD, order, penalty, nature, method, allHooks, dim)
            if resInterp is not None:
...
def miseAPlatDonnorTree__(zones, tc, procDict=None, procList=None):
    ...
    for z in zones_tc:
      ...
      for s in subRegions:
         ...
         if proc not in listproc: 
                ...
                sizeR.append(    Nbpts_InterpD + sizeIBC                 )
                ...
                sizeR[pos]    = sizeR[pos]     + Nbpts_InterpD + sizeIBC
    ...
    sizeproc  = []
    ntab_int  = 14
    for i in xrange(NbP2P): sizeproc.append(3 + ntab_int*rac[i] + sizeI[i])
    ...
    Internal.createUniqueChild(base, 'Parameter_int' , 'DataArray_t', param_int )
    Internal.createUniqueChild(base, 'Parameter_real', 'DataArray_t', param_real)
    ...
    for z in zones_tc:
      ...
      for s in subRegions:
            ...
            iadr = pt_ech + 2 + nb_rac[pos]   # ptr echange + dest + nrac + norac
            ...
            param_int[ iadr +rac[pos]*5 ] = c                          # No zone donneuse

            lst                              = pt_ech +3 +ntab_int*rac[pos] + sizeNbD[pos] + sizeType[pos] + size_ptlist[pos]  
            param_int[ iadr +rac[pos]*6    ] = lst                                                                      # PointlistAdr
            ptTy                             = pt_ech +3 +ntab_int*rac[pos] + sizeNbD[pos] + size_ptType[pos]
            param_int[ iadr +rac[pos]*7    ] = ptTy                                                                     # TypAdr
            lstD                             = pt_ech +3 +ntab_int*rac[pos] + size_ptlistD[pos]
            ...
            for ntype in Nbtype:
               ...
               for i in xrange(Nbpts_D):
                  ...
                  if ltype == ntype:
                       ...
                       if ntype != 0:
                         ...
                         #for kk in xrange(ncfLoc): param_int[  lst + noi + kk + 1] = pointlist[ 1][ noi_old + kk + 1]
                       ...
                       #recopie  InterpD
                       #for kk in xrange(sizecoef): param_real[ pt_coef + nocoef + kk ] = InterpD[1][ nocoef_old + kk]
               ...
               #print "ctyp", ptTy + ctyp +1 , ctyp,ntype,  l
            ...
            size_coef[pos] = size_coef[pos] + Nbpts_InterpD + size_IBC
...
def miseAPlatDonnorZone__(zones, tc, procDict):
	 ...
	 Internal.createUniqueChild(z, 'Parameter_real', 'DataArray_t', param_real)
...
def setSeqInterpolations(t, depth=2, double_wall=0, storage='direct',
    ...
    if double_wall == 1:
        ...
        firstWallCenters = dwInfo[0]; surfacesExtC = dwInfo[1]
    for nob in xrange(nbases):
        if (bases[nob][0] == 'CARTESIAN'): sameBase = 1
        zones = Internal.getNodesFromType1(bases[nob], 'Zone_t')
        nzones = len(zones)
        for noz in xrange(nzones):
            z = zones[noz]
            ...
            if (interpCells[1].size != 0): # pts interpoles
                # recuperation des domaines d interpolations
                res = getInterpolationDomains__(bases,nob,noz,zn,surfacest=surfacesExtC,sameBase=sameBase)
...
def createInterpRegion__(z, zname, pointlist, pointlistdonor, interpCoef, interpType, interpVol, indicesExtrap, indicesOrphan, \
    ...
    #elif itype == 'abutting': nameSubRegion = 'AD_'+zname
    elif itype == 'abutting': nameSubRegion = 'ID_'+zname
    else: nameSubRegion='IBCD_'+zname
    zsr = Internal.getNodesFromName1(z,nameSubRegion)
