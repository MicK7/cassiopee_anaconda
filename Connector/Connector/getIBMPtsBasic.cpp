...
PyObject* K_CONNECTOR::getIBMPtsBasic(PyObject* self, PyObject* args)
{
    ...
    E_Float he, hi; 
#ifdef E_DOUBLEREAL
    if (!PyArg_ParseTuple(args, "OOOdd", &allCorrectedPts, &normalNames, &distName, &hi, &he))
#else
    if (!PyArg_ParseTuple(args, "OOOff", &allCorrectedPts, &normalNames, &distName, &hi, &he))
#endif
        return NULL;
    // check distname
    char* distname;
    ...
    for (E_Int noz = 0; noz < nzones; noz++)
    {
        ...
        for (E_Int ind = 0; ind < npts; ind++)
        {
            ...
            // image pts
            if ( dist0 > K_CONST::E_GEOM_CUTOFF) delta = he;
            else
            {
                if ( hi <= K_CONST::E_GEOM_CUTOFF) delta = -dist0;//symmetrical if hi=0
                else delta = hi;
            }
