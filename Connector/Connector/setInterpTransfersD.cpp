...
PyObject* K_CONNECTOR::setInterpTransfersD(PyObject* self, PyObject* args)
{
  ...
  if (resd != 2 && resd != 1) 
  {
    PyErr_SetString(PyExc_TypeError,
                    "setInterpTransfersD: 1st arg is not a valid array.");
    return NULL; 
  }
...
PyObject* K_CONNECTOR::_setInterpTransfersD(PyObject* self, PyObject* args)
{
  ...
  if (!PYPARSETUPLEI(args,
                    "OOOOOllsss", "OOOOOiisss",
                    &zoneD,  &pyVariables, &pyIndDonor, &pyArrayTypes, &pyArrayCoefs, &vartype, &compact, 
...
# include "extract_interpD.h"
  ...
  if (compact==0)
  {
     ...
     if (initAll == true)
     {
       ...
       if (poscd == -1)
       {
         for (E_Int i = 0; i < nfld; i++) 
         {
           ...
           else {strcat(varStringOut,","); strcat(varStringOut,tmpvars[i]);}
         }
       }
       else
       {
     ...
     delete [] varStringD; delete [] eltTypeD;
...
PyObject* K_CONNECTOR::__setInterpTransfersD(PyObject* self, PyObject* args)
{
  ...
  E_Int loc, vartype, bctype, type_transfert,no_transfert;
  E_Float gamma, cv, muS, Cs, Ts;

  if (!PYPARSETUPLE(args,
                    "OOOOOllllddddd", "OOOOOiiiiddddd",
                    "OOOOOllllfffff", "OOOOOiiiifffff",
                    &zonesR, &zonesD, &pyVariables, &pyParam_int,  &pyParam_real, &vartype,
  ...
  if     (TypeTransfert==0) { pass_deb =1; pass_fin =2; }
  else if(TypeTransfert==1) { pass_deb =0; pass_fin =1; }
  else                      { pass_deb =0; pass_fin =2; }
  ...
  E_Int* ipt_ndimdxD; E_Int** ipt_cnd;
  E_Float** ipt_roD;  E_Float** ipt_roD_vert;
  ...
  char* varStringOut;
  PyObject** list_tpl;
  list_tpl = new PyObject*[nrac];

  E_Float** frp;  frp = new E_Float*[nrac];
  ...
  { 
    ...
    if (varname == "Density") // GROS BUG!!!! - CBX
    { 
      if   (nvars_loc == 5) varStringOut ="Density,VelocityX,VelocityY,VelocityZ,Temperature";
      else                  varStringOut ="Density,VelocityX,VelocityY,VelocityZ,Temperature,TurbulentSANuTilde";
    }
    else
    { 
      if   (nvars_loc == 5) varStringOut ="Density_P1,VelocityX_P1,VelocityY_P1,VelocityZ_P1,Temperature_P1";
      else                  varStringOut ="Density_P1,VelocityX_P1,VelocityY_P1,VelocityZ_P1,Temperature_P1,TurbulentSANuTilde_P1";
    }

    list_tpl[irac] = K_ARRAY::buildArray(nvars_loc, varStringOut, nbRcvPts, 1, 1);
    frp[irac]      = K_ARRAY::getFieldPtr( list_tpl[irac] );
  }
  ...
  FldArrayI rcvPtsI(nbRcvPts_mx); E_Int*  rcvPts = rcvPtsI.begin();
  ...
  //2eme pass: transfert
  //
    for  (E_Int ipass=pass_deb; ipass< pass_fin; ipass++)
    {
      for  (E_Int irac=0; irac< nrac; irac++)
      {        
        E_Int shift_rac =  ech + 2 + irac;
        E_Int ibc =  ipt_param_int[ shift_rac + nrac*3     ];
        //printf("ipass= %d, irac= %d, ibc=  %d, envoie vers: %d, size_rac= %d \n", ipass, irac, ibc, ipt_param_int[ ech ], ipt_param_int[ shift_rac + nrac*10 +1 ]);
        if( 1-ibc != ipass)  continue;
      ...
      if(loc == 0)
      {
        ...
        {
         vectOfRcvFields[eq] = frp[irac]          + eq*nbRcvPts;
      ...
      ////  
       ...
       E_Int nbInterpD;  E_Float* xPC; E_Float* xPI; E_Float* xPW; E_Float* densPtr;
       if(ibc ==1)
       { 
        nbInterpD = ipt_param_int[ shift_rac +  nrac   ];
...
/// oldschool
        ...
        { 
          ...
          //if( NoD==4 )  printf(" shift %d  %d %d %d  %d %d %d  %d \n", irac, NoR,NoD, ntype[ 1 + ndtyp],pt_deb,pt_fin , ipt_param_int[ shift_rac + nrac*8  ], ithread );
 ...
 */
  ...
  for  (E_Int irac=0; irac< nrac; irac++)
    { 
      ...
      PyList_Append(info ,  list_tpl[irac] ); Py_DECREF(list_tpl[irac]); //tableau data

      E_Int PtlistDonor = ipt_param_int[ shift_rac + nrac*12 +1];
      E_Int* ipt_listRcv = ipt_param_int+ PtlistDonor ;
      PyObject* listRcv  = K_NUMPY::buildNumpyArray( ipt_listRcv, nbRcvPts , 1, 1); 

      PyList_Append(info, listRcv); Py_DECREF(listRcv);   //ListReceveur

      PyList_Append(infos,  info); Py_DECREF(info);
    }


 delete [] list_tpl;
 delete [] frp;
 delete [] ipt_ndimdxD; delete [] ipt_roD; delete [] ipt_cnd;
