  ...
  for  (E_Int ipass=pass_deb; ipass< pass_fin; ipass++)
  {
    for  (E_Int irac=0; irac< nrac; irac++)
    {
      ...
      ////  
       ...
       E_Int nbInterpD=0;  E_Float* xPC=NULL; E_Float* xPI=NULL; E_Float* xPW=NULL; E_Float* densPtr=NULL;
       if(ibc ==1)
       { 
...
//        else          { pt_deb = ideb +    (chunk+1)*r+(nd-r)*chunk; pt_fin = pt_deb +  chunk;    }
      ...
      //printf(" irac= %d, NoR= %d, nvar=  %d, NoD= %d, Rans=  %d, deb= %d, fin= %d, ithread= %d \n", irac, NoR, nvars_loc, NoD, Rans ,pt_deb,pt_fin , ithread );
