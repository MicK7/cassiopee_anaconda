...
#ifdef E_DOUBLEINT 
    if (!PyArg_ParseTuple(args, "OOOOl", &allCorrectedPts, &bodySurfaces, &normalNames, &distName, &signOfDist))
#else
    if (!PyArg_ParseTuple(args, "OOOOi", &allCorrectedPts, &bodySurfaces, &normalNames, &distName, &signOfDist))
#endif
        return NULL;
