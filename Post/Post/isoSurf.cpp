                       ...
                       FldArrayF& fiso, FldArrayI& ciso)
{
  ...
  for (E_Int i = 0; i < nthreads; i++) cisos[i] = new FldArrayI(net,3);
  FldArrayF** fisos = new FldArrayF* [nthreads];
  for (E_Int i = 0; i < nthreads; i++) fisos[i] = new FldArrayF(3*net,nfld);
...
#pragma omp parallel default(shared)
  ...
  E_Int nptSize = 3*net; // init values
  E_Int ntriSize = net;
...
#pragma omp for
  for (E_Int i = 0; i < nelts; i++)
  {
    ...
    if (npt + 6 >= nptSize) 
    {nptSize += nelts; fisop.reAllocMat(nptSize, nfld);}
    if (ntri + 2 >= ntriSize)
    { ntriSize += nelts; cisop.reAllocMat(ntriSize, 3); 
      ciso1 = cisop.begin(1); ciso2 = cisop.begin(2); ciso3 = cisop.begin(3);
    }
  }
  ntris[ithread] = ntri;
  npts[ithread] = npt;
