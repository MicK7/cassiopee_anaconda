...
def writeEnvs():
     ...
     if cassiopee != '': envPath = os.path.join(libPath,'../../..')
     else: envPath = os.path.join(libPath,'../bin')
     cmdPath = os.path.join(libPath,'..')
     ...
     # sh
     p = open(os.path.join(envPath,"env_Cassiopee.sh"), 'w')
     ...
     # csh
     p = open(os.path.join(envPath,"env_Cassiopee.csh"), 'w')
     ...
     # bat
     p = open(os.path.join(envPath,"env_Cassiopee.bat"), 'w')
...
def getCppArgs():
    ...
    if Cppcompiler == "icc": 
         ...
         if useOMP() == 1:
            v = getCppVersion()
            if v[0] < 15: options += ['-openmp', '-fp-speculation=strict']
            else: options += ['-qopenmp', '-fp-model=precise']
