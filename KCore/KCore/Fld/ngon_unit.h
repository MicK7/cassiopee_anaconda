...
class ngon_unit
{
  public:
    ...
    ///
    void remove_facets(const Vector_t<E_Int>& facet_ids, Vector_t<E_Int>& nids); //0-based nids
    ...
    static void convert_fixed_stride_to_ngon_unit(const K_FLD::IntArray&cnt, ngon_unit& nguo);

    /// convert a n-to-one vector (tipically an oids) to a ngon_unit (WITH CONVENTION : molecule (1, E_IDX_NONE) for empty ones
    /// one-based upon exit
    void create_pgs_changes_graph(const Vector_t<E_Int>& oids, bool zero_based_in, E_Int shift, ngon_unit& nguo) const;
    
private:
    /// warning : need a call to updateFacets afterwards
    void __add (const ngon_unit& ng, E_Int ith);
    ///
    void __transfer_attributes(const ngon_unit& ngi, const Vector_t<E_Int>& nids);
    void __transfer_attributes(const ngon_unit& ngi, const Vector_t<bool>& keep);
