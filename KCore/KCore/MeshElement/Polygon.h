...
public:
  ...
  (const TriangulatorType& t, const K_FLD::FloatArray& coord, const E_Int* nodes, E_Int nb_nodes, E_Int index_start, K_FLD::IntArray& connectT3);//WARNING : connectT3 is Apended (not cleared upon entry)
...
///
template <typename TriangulatorType>
inline E_Int Polygon::triangulate(const TriangulatorType& t, const K_FLD::FloatArray& coord, K_FLD::IntArray& connectT3)
{
  return t.run(coord, _nodes, NB_NODES, connectT3, _shift, 1/*do ot shuffle*/);
}
...
///
  ...
  {
    // WARNING : append connectT3. not cleared
    ...
    switch (nb_nodes)
    {
      ...
      default: err = t.run(coord, nodes, nb_nodes, connectT3, shft, true/*do not shuffle*/);break; //indexing to -1 is done inside
    }
