...
public:
  ...
  static void shift (VecDec & vec, E_Int shift){for (size_t i=0;i<vec.size(); ++i)vec[i] += shift;}
  template<typename VecDec>
  static void shift (VecDec & vec, E_Int from, E_Int shift){for (size_t i=from;i<vec.size(); ++i)vec[i]+=shift;}

  template<typename VecDec>
  static void init_inc(VecDec & vec, E_Int sz){ vec.resize(sz, E_IDX_NONE); for( size_t i = 0; i < sz; ++i) vec[i] = i; }

  template <typename T>
  static void init_inc(K_FLD::DynArray<T>& arr, E_Int row, E_Int sz){ 
    arr.resize(arr.rows(), sz, E_IDX_NONE);  
    for (size_t i = 0; i < sz; ++i) arr(row, i) = i; }
