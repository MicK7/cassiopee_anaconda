                                 ...
                                 E_Float* cfLoc)
{
  ...
  // declaration de
  E_Float B[sizeBasis*nDnrPts];
  E_Float A[sizeBasis*sizeBasis];
  vector<E_Int> rows; // i | pi(0) !=0
  E_Float zeros[3];
  E_Int ok;

  // Matrix B = P.TW | (P.TW)ij = pi(xj)*W(X,xj)
  ok = matrixB(nDnrPts, sizeBasis, dnrIndices, xtDnr, ytDnr, ztDnr, order, dimPb, pt, radius, axis, axisConst, B);
  if ( ok == -1) return -4;

  // Matrix A = P.TW.P | (P.TW.P)ij = sum(k, 1, nDnrPts) pi(xk)*W(X,xk) pj(xk)
  //                      P.TW.P    = sum(k, 1, nDnrPts) p(xk) p(xk).T W(X,xk)
  ok = matrixA(nDnrPts, sizeBasis, dnrIndices, xtDnr, ytDnr, ztDnr, order, dimPb, pt, radius, axis, axisConst, A);
  if ( ok == -1) return -3;
  ...
  E_Float A_1[sizeBasis*rows.size()];  //Only rows such as basis(0) is not null of A^-1 will be computed

  // Inversion of A: compute rows of A^-1 such as basis(0)[row]!=0
  if (order >= 2)
  {
    ok = K_LINEAR::inv(sizeBasis, A, A_1, rows);
    if (ok == 0)
    {
...
{
  E_Float dnrIn[nDnrPts][3];
  E_Int ind, ok;

  for (E_Int ii = 0; ii < nDnrPts; ii++)
  {
    ind = dnrIndices[ii];
    dnrIn[ii][0] = xtDnr[ind];
    dnrIn[ii][1] = ytDnr[ind];
    dnrIn[ii][2] = ztDnr[ind];
  }
  ...
  for (E_Int i = 0; i < sizeBasis ; i++)
  {
    for (E_Int j = 0; j < nDnrPts; j++)
    {
      ...
      if ( ok == -1) return -1;
      B[i+j*sizeBasis] = basisLoc[i] *weightFunction(pt,dnrIn[j],radius,axis);
    }
  }
  return 1;
}
  ...
  for (E_Int i = 0; i < sizeBasis ; i++)
  {
    for (E_Int j = 0; j < sizeBasis; j++)
    {
      ...
      for (E_Int k = 0; k < nDnrPts; k++)
      {
        ...
        if ( ok != 1 ) return -1;
        sum += basis[i]*basis[j]*weightFunction(pt, dnrIn[k], radius, axis);
      }
      A[i+j*sizeBasis] = sum;
    }
  }
...

{
  ...
  E_Float xDnrIn[nDnrPts];
  E_Float yDnrIn[nDnrPts];
  E_Float zDnrIn[nDnrPts];
  E_Int ind;
  ...
  OBbox(dimPb, nDnrPts, xDnrIn, yDnrIn, zDnrIn, axis, bbox);
  ...
  E_Float *axis, E_Int *axisConst, vector<E_Int>& indicesIn)
{
  ...
  K_INTERP::matrixA(indicesIn.size(), sizeBasis, indicesIn,
                    xtDnr, ytDnr, ztDnr,
                    order, dimPb,
                    pt, radius, axis, axisConst, A);
  invOk = K_LINEAR::inv(sizeBasis, A, A_1, rows);
