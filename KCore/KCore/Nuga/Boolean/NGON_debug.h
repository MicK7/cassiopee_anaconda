...
(const char* fname, const ACoordinate_t& coord, const ngon_type& ng, const std::vector<bool>*keep, bool clean)
{
  ...
  MIO::write(fname, crd, cNGON, "NGON");
}
...
(const char* fname, const ACoordinate_t& coord, const ngon_type& ng)
{
  ...
  ng.PHs.extract_externals(PHex, oIds);
...
//    return;
  ...
  E_Int err = ngon_type::triangulate_ph(t, ng, PHi, coord, connectT3); // PH -> PHT3
...
void NGON_DBG_CLASS::draw_PGT3s(const K_FLD::FloatArray& coord,const ngon_unit& PGs)
{
  ...
  E_Int err = ngon_type::template triangulate_pgs<DELAUNAY::Triangulator>(PGs, coord, connectT3, T3_to_nPG);
...
(const K_FLD::FloatArray& coord,const ngon_unit& PGs, E_Int PGi, K_FLD::FloatArray* PGnormals)
{
  ...
  PGs.updateFacets();
  DELAUNAY::Triangulator t;
  t.run(coord, PGs.get_facets_ptr(PGi), PGs.stride(PGi), connectT3, 1, true/*do ot shuffle*/);
...
void NGON_DBG_CLASS::draw_PGs(const K_FLD::FloatArray& crd, const ngon_unit& PGs, Vector_t<E_Int> PGis, bool localid)
{
  for (size_t i = 0; i < PGis.size(); ++i)
    draw_PG(crd, PGs, PGis[i], localid);
}
/*
///
TEMPLATE_COORD_CONNECT
...
void NGON_DBG_CLASS::draw_PHs(const char* prefixname, const K_FLD::FloatArray& coord, const ngon_type& ng, const Vector_t<E_Int>& ids)
{
  ...
  {
    std::ostringstream o;
    o << prefixname << "_" << ids[i] << ".tp";
    draw_PH(o.str().c_str(), coord, ng, ids[i]);
  }
}
