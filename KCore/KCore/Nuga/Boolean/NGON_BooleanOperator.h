...
public : 
    ...
    enum eRetCode {EMPTY_X=-7, OK=0,ERROR=1};
...
private:
  ...
  ///
  void __flag_PHs_with_PGs(const ngon_type& wNG, Vector_t<bool>& keep, const Vector_t<E_Int>* pg_oids = 0);
...
, const K_FLD::IntArray& connectT3
  ...
  ///
  void __set_PH_history(const std::map<E_Int, Vector_t<E_Int> >& PHT3s, const Vector_t<E_Int>& is_skin, E_Int shift, E_Int nb_pgs1, const K_FLD::IntArray& F2E, K_FLD::IntArray& ancPH);
 ...
 private:
  ...
  E_Int zcount=0;
        ...
        molec_ph.clear();
          ...
          for (; itPG != Aggs.end(); ++itPG)
          {
            ...
            //  _enabled=true;
              ...
              //  MIO::write("pgt3.mesh", _coord, connectM, "TRI");
              ...
              {
#ifdef DEBUG_BOOLEAN
                //if (_enabled)MIO::write("connectT3.mesh", _coord, connectT3, "TRI");
#endif
                pgsi.clear();
                ...
                //NGON_DBG::write("pgagg.plt", ac, pgsi);
#endif
                ...
                anc[I]=hPG;
                for (size_t kk = 0; kk < pgsi.size(); ++kk)
          ...
          phs._ancEs.pushBack(_anc_PH_for_PHT3s.col(aphi), _anc_PH_for_PHT3s.col(aphi) + 2);
      }
	  ...
	  //#endif    
	  //  NGON_DBG::write("aggregated.plt", ac, ngout);  
	  //*/
#endif
  ...
  /// cleaning
  //
  void __clean_suprefulous_nodes(ngon_type& ng, const K_FLD::FloatArray& crd);
  ...
  ///
  E_Int __check_PHT3s_closure(const K_FLD::FloatArray& coord, const K_FLD::IntArray& connectT3, std::map<E_Int, Vector_t<E_Int> >& PHT3s);
  ...
  ///
  ///
  E_Int __discard_holes(const K_FLD::FloatArray& coord, ngon_type& wNG);
  ///
...
private:private:
  ...
  ACoordinate_t _aCoords1, _aCoords2;
public:
...
: _tolerance(tolerance), _convexity_tol(1.e-8), _AggPol(aggtype), _processed(false),
 _cNGON1(cNGON1), _cNGON2(cNGON2), _aCoords1(pos1, px, py, pz), _aCoords2(pos2, px2, py2, pz2)
{}
...
(const K_FLD::FloatArray& pos1, const K_FLD::IntArray& cNGON1,
 const K_FLD::FloatArray& pos2, const K_FLD::IntArray& cNGON2, E_Float tolerance, eAggregation aggtype)
  : _AggPol(aggtype), _tolerance(tolerance),  _convexity_tol(1.e-8), _processed(false),
 _cNGON1(cNGON1), _cNGON2(cNGON2), _aCoords1(pos1), _aCoords2(pos2)
{}
...
(Coordinate_t& coord, Connectivity_t& connect, eInterPolicy XPol, eMergePolicy MPol)
{
  ...
  else if (ret == EMPTY_X)
  {
    ...
    coord.pushBack(_aCoords2.array());
...
#ifdef DEBUG_BOOLEAN // clean is done outside in Release soo need to do it inside for debug.
  if (ret != ERROR)
  {
...
#ifndef WIN32
    MIO::write("U.plt", coord, tmp, "NGON");
#else
    MIO::write("U.tp", coord, tmp, "NGON");
#endif
    std::cout << "total number of elements " << connect(0, 0) << std::endl;
  }
  else
...
(E_Int rec_op, std::vector<E_Int>& rec_ids, std::vector<E_Int>& xdon, std::vector<E_Int>& don_ids, std::vector<E_Float>& don_coefs)
{
  ...
  const K_FLD::FloatArray & crd_don = (rec_op==0) ? _aCoords2.array() : _aCoords1.array();
  const K_FLD::FloatArray & crd_rec = (rec_op==0) ? _aCoords1.array() : _aCoords2.array();
...
//  }
  ...
  for (it = recId_to_bitIds.begin(); it != recId_to_bitIds.end(); ++it, ++count)
  {
    ...
    K_MESH::Polyhedron<STAR_SHAPED>::volume<DELAUNAY::Triangulator>(dt, crd_rec, ng_rec, rec_PHi, v_rec, Gdum); //volume of donnor ancestor to compute the fraction
    ...
    for (size_t i=0; i < nb_sources; ++i)
    {
      ...
      E_Float v;
      K_MESH::Polyhedron<STAR_SHAPED>::volume<DELAUNAY::Triangulator>(dt, coord, *_ngoper, PHibit, v, Gdum); //volume of the piece of donnor PH
      vcumul += v;
...
(E_Int rec_op, std::vector<E_Int>& rec_ids, std::vector<E_Int>& xdon, std::vector<E_Int>& don_ids, std::vector<E_Float>& don_coefs, K_FLD::FloatArray& piece_centroids, K_FLD::FloatArray& crd, ngon_type& ngoper, std::vector<E_Int>& piece_ids)
{
  ...
  K_MESH::Polyhedron<STAR_SHAPED>::centroids<DELAUNAY::Triangulator>(*_ngoper, coord, pcentroids);
  ...
  for (it = recId_to_bitIds.begin(); it != recId_to_bitIds.end(); ++it, ++count)
  {
    ...
    K_MESH::Polyhedron<STAR_SHAPED>::volume<DELAUNAY::Triangulator>(dt, crd_rec, ng_rec, rec_PHi, v_rec, Gdum); //volume of receptor to compute the fraction
...
(E_Int rec_op /*0 or 1*/, const K_FLD::FloatArray& fields_don, K_FLD::FloatArray& fields_rec, E_Int field/*-1 means all*/)
{
  ...
  const K_FLD::FloatArray & dcrd = (rec_op==0) ? _aCoords2.array() : _aCoords1.array(); // donnor coords
  const K_FLD::FloatArray & rcrd = (rec_op==0) ? _aCoords1.array() : _aCoords2.array(); //receptor coords
...
(E_Int rec_op /*0 or 1*/, const K_FLD::FloatArray& fields_don, const K_FLD::FloatArray& grads_don, K_FLD::FloatArray& fields_rec, E_Int field/*-1 means all*/)
{
  ...
  K_MESH::Polyhedron<STAR_SHAPED>::centroids<DELAUNAY::Triangulator>(ngd, dcrd, don_centroids);
...
//#ifdef DEBUG_BOOLEAN
  ...
  K_MESH::Polyhedron<STAR_SHAPED>::volumes<DELAUNAY::Triangulator>(dcrd, ngd, dvols);
  std::vector<E_Float> rvols;
  K_MESH::Polyhedron<STAR_SHAPED>::volumes<DELAUNAY::Triangulator>(rcrd, ngr, rvols);
  std::vector<E_Float> pvols;
  K_MESH::Polyhedron<STAR_SHAPED>::volumes<DELAUNAY::Triangulator>(pcrd, ngoper, pvols);
  ...
  K_FLD::FloatArray rec_centroids;
  K_MESH::Polyhedron<STAR_SHAPED>::centroids<DELAUNAY::Triangulator>(ngr, rcrd, rec_centroids);
...
#ifdef DEBUG_BOOLEAN
        E_Float vv, GG[3];
        K_MESH::Polyhedron<STAR_SHAPED>::volume<DELAUNAY::Triangulator>(dt, pcrd, ngoper, pids[k], vv, GG);
...
E_Int NGON_BOOLEAN_CLASS::XcellN (std::vector<E_Float>& xcellN)
{
  ...
  xcellN.resize(ng1.PHs.size(), 0.); 
  
  //
  Coordinate_t coord;
  Connectivity_t connect;
  Diff(coord, connect, SOLID_RIGHT, PRESERVE_RIGHT);
...
//  }
  ...
  for (size_t PHi=0; PHi < nb_phs; ++PHi)
  {
    ...
    E_Float vold, Gdum[3];
    K_MESH::Polyhedron<STAR_SHAPED>::volume<DELAUNAY::Triangulator>(dt, crd1, ng1, ancPH1i, vold, Gdum);
    E_Float vnew;
    K_MESH::Polyhedron<STAR_SHAPED>::volume<DELAUNAY::Triangulator>(dt, coord, *_ngoper, PHi, vnew, Gdum);
    
    xcellN[ancPH1i] = vnew/vold;
    xcellN[ancPH1i]=std::min(xcellN[ancPH1i], 1.); // cutoff to 1.
    
    //if (xcellN[ancPH1i] > 0. && xcellN[ancPH1i] < 1.)
      //std::cout << "one partial value : " << vnew/vold << std::endl;
    
    assert(xcellN[ancPH1i] > 0.);
  }
  
#ifdef DEBUG_BOOLEAN

//  for (size_t i=0; i < cellN.size(); ++i)
//    if (cellN[i] != 0. && cellN[i] != 1.)
//      std::cout << "celln : " << cellN[i] << std::endl;
  size_t sz = xcellN.size();
  std::vector<bool> keep(xcellN.size(), false);
  for (size_t i=0; i < xcellN.size(); ++i)
  {
    keep[i]=((0. < xcellN[i]) && (xcellN[i] < 1.));
    //if (keep[i])
      //std::cout << "should keep : " << i << std::endl;
  }
  ...
  MIO::write("kept.plt", crd1, cnto, "NGON");
  
  ng1.select_phs(*_ngoper, keeinope, npgids, bngo);
  ngo.PHs=bngo.PHs;
  ngo.PGs = bngo.PGs;
  ngo.export_to_array(cnto);
  MIO::write("curcelllayer.plt", coord, cnto, "NGON");
#endif
...
(eInterPolicy XPol, eMergePolicy MPol, ngon_unit& wPGs, E_Int& nb_pgs1, ngon_unit& extrawPGs)
{
  ...
  nb_modifs += ngon_type::clean_connectivity(wNG2, _aCoords2.array());
...
#ifdef FLAG_STEP
  if (chrono::verbose >0) std::cout << "__get_working_PGs : flag externals : " << c.elapsed() << std::endl;
  c.start();
#endif
  ...
  if (wNG2.PHs.size() != wNG2.PGs.size())//if it's not a surface : partial easy filter before the right one based on conform-ngon-or-not
    ngon_type::close_phs(wNG2, _aCoords2.array());
  ...
  if (XPol==SOLID_RIGHT) // discard inner holes. Is there an application requiring them ? fixme : something has to be done for BOTH_SURFACE and SURFACE_RIGHT modes.
    __discard_holes(_aCoords2.array(), wNG2);
...
//#ifdef DEBUG_BOOLEAN
//{
//  char* fname1 = "working1.plt";
//  NGON_DBG::write(fname1, _aCoords1, wNG1);
//  char* fname2 = "left1.plt";
...
//#endif
  ...
  if ((XPol != SURFACE_RIGHT) && (XPol != BOTH_SURFACE)) // i.e. if the second operand is not just a surface
  {
   if (nb_ph20 != wNG2.PHs.size())
   {
     ngon_type tmp(wNG2);
     tmp.flag_externals(INITIAL_SKIN);
     //PGs
...
{
  K_FLD::ArrayAccessor<Coordinate_t> ac(_coord);
  char* fname1 = "working1g.plt";
  NGON_DBG::write(fname1, ac, wNG1);
  char* fname2 = "left1g.plt";
  NGON_DBG::write(fname2, ac, _ng1);
  fname1 = "working2g.plt";
  ...
  K_FLD::IntArray F2E2;
  {
    ...
    // update to make reference to global PH ids
    for  (size_t i=0; i < _F2E.cols(); ++i) 
    {
      _F2E(0,i)=wNG1.PHs._ancEs(0,_F2E(0,i));
      _F2E(1, i) = (_F2E(1, i) != E_IDX_NONE) ? wNG1.PHs._ancEs(0, _F2E(1, i)) : E_IDX_NONE;
    }
    ...
    // update to make reference to global PH ids
    for  (size_t i=0; i < F2E2.cols(); ++i) 
    {
      F2E2(0, i) = wNG2.PHs._ancEs(1, F2E2(0, i));
      F2E2(1, i) = (F2E2(1, i) != E_IDX_NONE) ? wNG2.PHs._ancEs(1, F2E2(1, i)) : E_IDX_NONE;
    }
  }
...
//  {
//  K_FLD::ArrayAccessor<Coordinate_t> ac(_coord);
//  char* fname1 = "working1h.plt";
//  NGON_DBG::write(fname1, ac, wNG1);
//  char* fname2 = "left1h.plt";
//  NGON_DBG::write(fname2, ac, _ng1);
//  fname1 = "working2h.plt";
//  NGON_DBG::write(fname1, ac, wNG2);
//  fname2 = "left2h.plt";
//  NGON_DBG::write(fname2, ac, _ng2);
//
...
//  }
  ...
  if (XPol == SOLID_RIGHT)
  {
    ...
    for (size_t i = 0; i < F2Etmp.cols(); ++i)
    {
...
#ifdef DEBUG_BOOLEAN
{
  char* fname1 = "working.plt";
  K_FLD::ArrayAccessor<Coordinate_t> ac(_coord);
  NGON_DBG::write(fname1, ac, wPGs);
  char* fname11 = "extra.plt";
...
NGON_BOOLEAN_CLASS::__compute()
{
  ...
  if (_Op != MODIFIED_SOLID)
  {
...
#ifdef DEBUG_BOOLEAN
    {
      ...
      MIO::write("is_skin.mesh", _coord, connectT3, "TRI", 0, &colors);
    }    
#endif
...
, connectT3o
    ...
    __clean_suprefulous_nodes(_ngXs, _coord);
...
#ifdef DEBUG_BOOLEAN
  NGON_DBG::write("ngXs.plt", ACoordinate_t(_coord), _ngXs);
#endif
  ...
  if (_build_hard)
  {
    ...
    __set_PH_history(PHT3s, is_skin, shift, -1, _extraF2E, _anc_PH_for_PHT3s);
    ...
    __clean_suprefulous_nodes(_ngXh, _coord);
  }
...
#ifdef DEBUG_BOOLEAN
  //NGON_DBG::write("ngXsG.plt", ACoordinate_t(_coord), _ngXs);
  //NGON_DBG::write("ngXhG.plt", ACoordinate_t(_coord), _ngXh);
  //NGON_DBG::write_external_phs("ngXsG_ex.plt", ACoordinate_t(_coord), _ngXs);
  //std::vector<bool> keep(_ngXs.PHs.size(), true);
  //for (size_t i=0; i < _ngXs.PHs.size(); ++i) keep[i]=(_ngXs.PHs._external[i] == false);
  //NGON_DBG::write("ngXsGin.plt", ACoordinate_t(_coord), _ngXs, &keep);
#endif
...
{
 #ifdef FLAG_STEP
  std::cout << "NGON Boolean : nb points : "  << _aCoords1.size() + _aCoords2.size() << std::endl;
  ...
  E_Int err = ngon_type::template triangulate_pgs<DELAUNAY::Triangulator>(wPGs, _coord, connectT3, oT3_to_PG);
  if (err)
    return ERROR;
 ...
 K_FLD::IntArray& connectHard, Vector_t<E_Int>& nT3_to_PG, Vector_t<E_Int>& is_skin)
{
  ...
  K_FLD::IntArray cT3;
  E_Int err = ngon_type::template triangulate_pgs<DELAUNAY::Triangulator>(extrawPGs, _coord, cT3, oT3_to_PG);
  if (err) return err;
...
(const K_FLD::IntArray& connectT3, const Vector_t<E_Int>& nT3_to_PG, ngon_unit& extrawPGs)
{
  ...
  {  
    ...
    node_to_nodes.clear();
    E_Int nb_bounds(boundaries.size()), nb_free(0);
    bool free_consecutive = false;
    for (E_Int i = 0; i < nb_bounds; ++i)
    {
...
NGON_BOOLEAN_CLASS::__reorient_externals(eInterPolicy XPol, ngon_type& wNG1, ngon_type& wNG2)
{
  ...
  if ((XPol == SURFACE_RIGHT) || (XPol == BOTH_SURFACE))
  {
    ...
    bool has_been_rev;
    E_Int er = ngon_type::reorient_skins(dt, _aCoords2.array(), wNG2, has_been_rev);
    if (er)
      return ERROR;
...
#ifdef DEBUG_BOOLEAN
{
  const char* fname1 = "working2_E1.plt";
  NGON_DBG::write(fname1, _aCoords2, wNG2);
  const char* fname2 = "left2_E1.plt";
  NGON_DBG::write(fname2, _aCoords2, _ng2);
}
#endif
  }
  ...
  if (XPol == SOLID_RIGHT)
    __create_pg_boxes(wNG2, _aCoords2, boxes2, pool2, GBbox2, &pg_oids, true/*only_skin*/);
  else if ((XPol == SURFACE_RIGHT) || (XPol == BOTH_SURFACE))
    __create_pg_boxes(wNG2, _aCoords2, boxes2, pool2, GBbox2);
  else
    __create_ph_boxes(wNG2, _aCoords2, boxes2, pool2, GBbox2);
  ...
  // E3 : 
  {
    ...
    if (sz1 == 0) //nothing has been capured
      return EMPTY_X;
  ...
  if ((XPol == SOLID_RIGHT) || (XPol == SURFACE_RIGHT) || (XPol == BOTH_SURFACE)) // i.e. wNG2 is_in2 is sized for PGs
    __flag_PHs_with_PGs(wNG2, is_in2, &pg_oids);

  if (XPol == BOTH_SURFACE)
    __flag_PHs_with_PGs(wNG1, is_in1);
...
#ifdef DEBUG_BOOLEAN
  {
	char* fname1 = "working1_E3.plt";
	NGON_DBG::write(fname1, _aCoords1, wNG1);
	char* fname2 = "left1_E3.plt";
	NGON_DBG::write(fname2, _aCoords1, _ng1);
	fname1 = "working2_E3.plt";
	NGON_DBG::write(fname1, _aCoords2, wNG2);
	fname2 = "left2_E3.plt";
	NGON_DBG::write(fname2, _aCoords2, _ng2);
  }
...
///
TEMPLATE_COORD_CONNECT
void NGON_BOOLEAN_CLASS::__flag_PHs_with_PGs(const ngon_type& wNG, Vector_t<bool>& keep, const Vector_t<E_Int>* pg_oids)
{
  Vector_t<bool> tmp;
 
  if (pg_oids && !pg_oids->empty()) // from reduced pg set to initial
  {
    assert (keep.size() == pg_oids->size());
    tmp.resize(wNG.PGs.size(), false);
    E_Int nb_reduce_pgs = pg_oids->size();
    for (E_Int i=0; i < nb_reduce_pgs; ++i)
      tmp[(*pg_oids)[i]]=keep[i];
    keep=tmp;
  }
    
  wNG.flag_PHs_having_PGs(keep, tmp); // from PGs to PHs
  keep=tmp;
}
...
///
TEMPLATE_COORD_CONNECT
E_Int NGON_BOOLEAN_CLASS::__discard_holes(const K_FLD::FloatArray& coord, ngon_type& wNG)
{
  ...
  ngon_unit neighbors;
  ngon_type::build_pg_neighborhood(pg_ext, neighbors);
  ...
  E_Int Ci=0;
  for (E_Int i=1; i < nb_connex; ++i)
  {
    if (
        (bbox_per_color[i].minB[0] <= bbox_per_color[Ci].minB[0]) && (bbox_per_color[i].maxB[0] >= bbox_per_color[Ci].maxB[0]) &&
        (bbox_per_color[i].minB[1] <= bbox_per_color[Ci].minB[1]) && (bbox_per_color[i].maxB[1] >= bbox_per_color[Ci].maxB[1]) &&
        (bbox_per_color[i].minB[2] <= bbox_per_color[Ci].minB[2]) && (bbox_per_color[i].maxB[2] >= bbox_per_color[Ci].maxB[2])
       )
      Ci=i;
  }
...
//  //NGON_DBG::get_PHT3_neighbors(837, PHT3s, coord, connectT3o, neighbors, true/*both_orient*/);
  ...
  PHierr = __check_PHT3s_closure(coord, connectT3o, PHT3s);
  ...
  if (PHierr != -1)
  {
    std::cout << " faulty PHT3 is : " << PHierr << "(" << PHT3s[PHierr].size() << " T3s)" << std::endl;
#ifdef DEBUG_BOOLEAN
...
, connectT3o
  ...
  __set_PH_history(PHT3s, is_skin, shift, _nb_pgs1, _F2E, _anc_PH_for_PHT3s);
...
E_Int NGON_BOOLEAN_CLASS::__classify_soft()
{
  ...
  NGON_DBG::write("walls.plt", ACoordinate_t(_coord), pg_ext);
  ...
  NGON_DBG::write("ext_from_neigh.plt", ACoordinate_t(_coord), _ngXs, &external);
  K_CONNECT::IdTool::negative(external);
  NGON_DBG::write("in_from_neigh.plt", ACoordinate_t(_coord), _ngXs, &external);
  
  // check is external PGs are OK
  ngon_unit pgex;
  std::vector<E_Int> oids;
  _ngXs.PGs.extract_externals(pgex, oids);
  NGON_DBG::write("pgex.plt", ACoordinate_t(_coord), pgex);
  */
#endif
...
/*
{
  ...
  NGON_DBG::write("toto.plt", ACoordinate_t(_coord), _ngXs, &keep);
}
*/
#endif
#endif
...
#ifdef DEBUG_BOOLEAN
  //NGON_DBG::write("wrong_col.plt", ACoordinate_t(_coord), _ngXs, &keep);
#endif
...
#ifdef DEBUG_BOOLEAN
    if (Zi== Z_1)
      NGON_DBG::write("first.plt", ACoordinate_t(_coord), *ngi);
    else if (Zi == Z_2)
      NGON_DBG::write("second.plt", ACoordinate_t(_coord), *ngi);
    else if (Zi == Z_IN)
      NGON_DBG::write("in.plt", ACoordinate_t(_coord), *ngi);
...
(const K_FLD::FloatArray& coord, const K_FLD::IntArray& connectT3, const K_FLD::IntArray& connectT3o, K_FLD::IntArray& neighbors)
{
  ...
  E_Int K0, E0, E1, A, N1, sz, Kb, i, j, shift(connectT3.cols());
  //E_Float q, qb; 
...
(const K_FLD::FloatArray& coord, const K_FLD::IntArray& connectT3o, std::map<E_Int, Vector_t<E_Int> >& PHT3s)
{
  ...
  for (iterator i = PHT3s.begin(); (i != PHT3s.end()) ; ++i)
  {
    ...
    sz = T3s.size();
    if (sz < 3) //cannot be a closed PHT3s shape, so discard it
    {
  ...
  {
#ifdef DEBUG_BOOLEAN
    //draw_PHT3(coord, connectT3, PHT3s, ii);
#endif
    PHT3s.erase(ii);
  }
  ...
  for (it = PH_to_PGT3s.begin(); it != PH_to_PGT3s.end(); ++it)
  {
    ...
    for (itPG = PGT3s.begin(); itPG != PGT3s.end(); ++itPG)
    {
      ...
      for (size_t i=0; i < szT3; ++i)
      {
        ...
        if (PHcur == PHopp)
          PH_to_PGT3s[PHi][PGi].push_back(T3scopy[i]); //use the initial entry for color PHcur
        else
          PH_to_PGT3s[PHi][maxPGId+PHopp].push_back(T3scopy[i]); //build unique id (maxPGId + PHopp) for new entries
      }
    }
  }
...
///
TEMPLATE_COORD_CONNECT
template <typename NGON_BOOLEAN_CLASS::eAggregation AGG>
E_Int NGON_BOOLEAN_CLASS::__aggregate_PHs
(const K_FLD::IntArray& connectT3, const Vector_t<E_Int>& is_skinT3,
 const std::map<E_Int, std::map<E_Int, Vector_t<E_Int> > >& PH_to_PGT3s, ngon_type& ngout)
{        
#ifdef FLAG_STEP
  chrono c;
  c.start();
  ...
  for (it = PH_to_PGT3s.begin(); it != PH_to_PGT3s.end(); ++it)
  {
    ...
    for (; itPG != Aggs.end(); ++itPG)
    {
      ...
      if (itPHbits == key_to_phbits.end()) // Not already aggregated
      {
        ...
        pgs.append(pgsi);
        pgs._type.resize(pgs.size(), is_skin ? INITIAL_SKIN : INNER);//IMPORTANT : use flag to mark skin PGs (for soft)
        _nPG_to_oPG.resize(pgs.size(), PGi);
...
#ifdef DEBUG_BOOLEAN
  {
    ...
    for (size_t i=0; i < ngout.PHs.size(); ++i)
    {
      //std::cout << i << std::endl;
      //NGON_DBG::draw_PH("pho.plt", _coord, ngout, i);
      assert(ngon_type::is_closed(ngout, i, free_edges));
    }
  }
//
//#ifdef FLDARR
       ...
       std::deque<E_Int>& sorted_contour, E_Int& K0, E_Int& n0, E_Int& Eip1)
{
  ...
  for (size_t i = 0; i < boundaries.size(); ++i)
  {
    ...
    */
    ...
    if (uu < - _convexity_tol) //concavity
    {
      convex = false;
      ps = K_FUNC::dot<3>(Ei, Ej);
      if ((K0 == E_IDX_NONE) || (ps < worst_ps))
      {
...
(const std::map<E_Int, Vector_t<E_Int> >& PHT3s, const Vector_t<E_Int>& is_skin, E_Int shift, E_Int nb_pgs1, const K_FLD::IntArray& F2E, K_FLD::IntArray& anc_PH)
{
  ...
  for (it = PHT3s.begin(); it != PHT3s.end(); ++it, ++i)
  {
    ...
    for (size_t j = 0; (j< sz); ++j)
    {
      ...
      if ((_XPol == SOLID_RIGHT) && (wPG >= nb_pgs1)) // hard skin with out element
        continue;
...
//
TEMPLATE_COORD_CONNECT
void NGON_BOOLEAN_CLASS::__clean_suprefulous_nodes(ngon_type& ng, const K_FLD::FloatArray& crd)
{  
  return ;
  Vector_t<bool> man_nodes;

  ngon_type::flag_manifold_nodes(ng, crd, man_nodes);

  Vector_t<E_Int> nids, node_ids(crd.cols());//0-based ids
  for (E_Int i = 0; i < crd.cols(); ++i) node_ids[i] = (man_nodes[i]) ? E_IDX_NONE : i;
  
  Vector_t<E_Int> flag = ng.PGs._type; //hack
  K_FLD::IntArray anc = ng.PGs._ancEs; //hack
  
  ng.PGs.remove_facets(node_ids, nids);
  
  ng.PGs._type.resize(ng.PGs.size());
  for (size_t i=0; i<nids.size(); ++i)
    ng.PGs._type[nids[i]] = flag[i];

  ng.PGs._ancEs.resize(2, ng.PGs.size());
  for (size_t i = 0; i < nids.size(); ++i)
  {
    ng.PGs._ancEs(0, nids[i]) = anc(0, i);
    ng.PGs._ancEs(1, nids[i]) = anc(1, i);
  }
}
