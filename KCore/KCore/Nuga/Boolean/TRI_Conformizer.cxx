 ...
 K_CONT_DEF::int_vector_type& ancestors)
{
  ...
  mode.do_not_shuffle = true; // option to speed up.
      ...
      // swap ? if original edge are present in the triangulation whereas they got splitting points
      {
        ...
        for (size_t t=0; t < 3; ++t)
        {
          ...
          if (sz11> 2) // has splitting points
            origE.insert(K_MESH::NO_Edge(_edges[tri.edges[t]][0], _edges[tri.edges[t]][sz11-1])); //insert the original edge
        }
        if (!origE.empty())
        {
          ...
          for (E_Int k=0; k < data.connectM.cols(); ++k)
          {
            ...
            for (size_t n=0; n < 3; ++n)
            {
              ...
              if (itE != origE.end())
              {
                swapE.push_back(std::make_pair(k,(n+2)%3));
                origE.erase(itE);
              }
            } 
          }
        ...
        K_CONNECT::GeomAlgo<K_MESH::Triangle>::get_swapE(*data.pos, data.connectM, data.neighbors, data.hardEdges, parent_type::_tolerance, swapE); //warning : quality<2> doesnt give the same result as quality<3>. need to convert to tolerance criterium.


        if (!swapE.empty())
          err = K_CONNECT::EltAlgo<K_MESH::Triangle>::fast_swap_edges(swapE, data.connectM, data.neighbors);
      }
